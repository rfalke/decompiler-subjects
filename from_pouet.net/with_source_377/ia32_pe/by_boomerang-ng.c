...
__size32 proc_0x00401017(unsigned int param1);
...
__size32 proc_0x00401017(unsigned int param1)
{
    ...
    int local3; 		// edi_7{0}
    unsigned int local4; 		// param1{0}
    int local5; 		// edx{0}
    int local6; 		// edi{0}
    unsigned int local7; 		// local0{0}
    ...
    *(unsigned char*)edi_1 = global_0x00402136;
    ...
    do {
        ...
        edi_7 = local3;
        param1 = local4;
        ecx_2 = ecx_1 >> 8 & 0xffffff | 2;
        dx = 1275;
        dh = 1275 >> 8 & 0xff;
        dl = (unsigned char) edx_1;
        edx_2 = edx_1 >> 16 & 0xffff | 1275;
        local1 = ecx_2;
        local5 = edx_2;
        local5 = edx_2;
        local6 = edi_7;
        ...
        if ( ~(bl > (unsigned char) edx_1 || bl < (1275 >> 8 & 0xff))) {
            ...
            bx = (bx >> 8 & 0xff | (bl >> (ecx_1 >> 8 & 0xffffff | 2))) & 0xff | bh * 256;
            di = edi_1 + bx;
            edi_5 = ((edi_7 >> 16 & 0xffff | ((bh))) >> 16 & 0xffff | ((bh) * 64)) >> 16 & 0xffff | (edi_1 + bx);
            ...
            local7 = local0;
            if (tmpb == 0) {
                edx = (edx_1 >> 16 & 0xffff | 1275) >> 16 & 0xffff | ((unsigned short) ebx_1);
                dh = ((unsigned short) ebx_1 & 0xf0f) >> 8 & 0xff;
                dx = ((unsigned short) ebx_1 & 0xf0f) >> 8 & 0xff | (edi_1 + dh);
                edx = (((edx_1 >> 16 & 0xffff | 1275) >> 16 & 0xffff | ((unsigned short) ebx_1)) >> 16 & 0xffff | ((unsigned short) ebx_1 & 0xf0f)) >> 8 & 0xffffff | (edi_1 + dh);
                dx = (((unsigned short) ebx_1 & 0xf0f) >> 8 & 0xff | (edi_1 + dh)) << (ecx_1 >> 8 & 0xffffff | 2);
                dh = dx >> 8 & 0xff;
                dl = (unsigned char) edx;
                edx = edx >> 16 & 0xffff | (dx);
                local5 = edx;
            }
        }
        edx = local5;
    ...
    return 0; /* WARNING: Also returning: dx_1 := dx, bx := (unsigned short) ebx_1 - 1, di := di, al := 0, cl := 2, dl := dl, bl_1 := (unsigned char) ebx_1 - 1, dh := dh, bh_1 := ebx_1 - 1 >> 8 & 0xff, ecx := ecx_1 >> 8 & 0xffffff | 2, edx := edx, ebx := ebx_1 - 1, ebp := 0, esi := global_0x0040211c, edi := edi */
}
