...
void fn0C00_0100(byte ch, struct Eq_3 * ds)
{
	...
	do
	{
		__outb(969, (byte) ax_371);
		__outb(969, 0x3F);
		__outb(969, 0x3F);
		--cx_273;
		byte ah_271 = SLICE(ax_371 + 0x01, byte, 8);
	...
	while (true)
	{
		...
		do
		{
			...
			uint32 dx_ax_132 = (uint32) (di_118 - 0x01);
			...
			if (SEQ(ah_175, __inb(0x60)) == 0x01)
				return;
			--cx_197;
			byte cl_305 = (byte) cx_197;
		} while (cx_197 != 0x00);
		byte ah_226 = 0x00;
		word16 cx_219 = SEQ(~0x05, cl_305);
		do
		{
			struct Eq_199 Eq_125::* si_221 = cx_219 - 0x01;
