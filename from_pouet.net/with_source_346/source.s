...
@@x:    push    cx
        ...
        mov     cx,[si]
        mov     dx,[si+(GRIDXSIZE+1)*6]
        sub     dx,cx
        sar     dx,GRIDSHIFT
        mov     lvslope2,dx

        mov     cx,[si+6]
        mov     dx,[si+(GRIDXSIZE+1)*6+6]
        sub     dx,cx
        sar     dx,GRIDSHIFT
        mov     rvslope2,dx
...
message         db      'TBLish indeed,no? :)',13,10,'$'

f_flowermorphsinpos dd  0
f_flowermorphsinpos_add dd 0.02

f_1_over_gridxsize dd   3ccccccdh
f_1_over_gridysize dd   3d23d70ah
f_2pi_to_texwidth dd    4622f983h
