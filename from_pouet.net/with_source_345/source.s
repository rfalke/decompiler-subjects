...
PShader:                       	db      "float f;"
				...
				db	"{"
				db		"float2 c=v/float2(640,360)-1,p=0,n;"
   				db		"float q=f*.0001;"
				db		"q+=sin(q*3.+cos(q*4.))*.5;"
				db		"v.x=(cos(q)*c.x+sin(q)*c.y)*640/360;"
				db		"v.y=sin(q)*c.x-cos(q)*c.y;c=v;"
    				db		"c/=abs(c.y+1.33)+.5+sin(f*.0003)*.25;"
 				db 		"c*=float2(1,1)*.025;"
				db		"c+=float2(-1+.01,0);"
				db		"float4 d=0;"
    				db		"float mR2=.11;"
    				db		"float sc=2.85+cos(f*.000077)*.5-.5;"
				db		"for(int i=0;i<120;i++)"
				db		"{"
 				db		     	"p=clamp(p,-1,1)*2-p;"
      				db			"float r2=dot(p,p);"
 				db		 	"p*=clamp(max(mR2/r2,mR2),0,1);"
				db			"p=p*sc+c;"
      				db			"n.x=p.x*p.x-p.y*p.y;"
				db			"n.y=2*p.x*p.y;"
 				db			"p=n+c;"
				db			"d+=float4(.3,.6,.1,0)*s(p,.25,.25,.4,.425);"
				db			"d+=float4(.3,.2,.1,0)*s(p,-.25,.25,.5,.55);"
				db			"d+=float4(.4,.2,.1,0)*s(p,-.35,-.45,.6,.725);"
				db			"d+=float4(.1,.4,.3,0)*s(p,.35,.5,.6,.725);"
				db			"d+=float4(.6,.2,.1,0)*s(p,.75,.15,.5,.525);"
				db			"d+=float4(1,1,1,0)*s(p,.15,.25,.4,.425);"
				db			"d+=float4(0,1,1,0)*s(p,.45,.75,.8,.8125);"
				db		"}"
				db		"d/=log(600)*600/250;"
				db		"return d*min(f*0.0001-.2,1);"
				db	"}"
fPShader:
