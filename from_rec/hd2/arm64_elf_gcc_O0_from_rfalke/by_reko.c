// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_fini.h"

// 0000000000000E18: void _fini()
void _fini()
{
}

// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_init.h"

// 00000000000007E8: Register word64 _init(Register out ptr64 x30Out)
// Called from:
//      __libc_csu_init
word64 _init(ptr64 & x30Out)
{
	call_weak_fn();
	x30Out = qwArg00;
	return x30;
}

// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_plt.h"

// 0000000000000820: void fn0000000000000820()
// Called from:
//      dumpline
void fn0000000000000820()
{
	g_ptr12000();
}

// 0000000000000830: void fn0000000000000830()
// Called from:
//      hexdump
void fn0000000000000830()
{
	g_ptr12008();
}

// 0000000000000840: void fn0000000000000840()
// Called from:
//      __do_global_dtors_aux
void fn0000000000000840()
{
	g_ptr12010();
}

// 0000000000000850: void fn0000000000000850()
// Called from:
//      dumpline
void fn0000000000000850()
{
	g_ptr12018();
}

// 0000000000000860: void fn0000000000000860()
// Called from:
//      hexdump
void fn0000000000000860()
{
	g_ptr12020();
}

// 0000000000000870: void fn0000000000000870()
// Called from:
//      hexdump
void fn0000000000000870()
{
	g_ptr12028();
}

// 0000000000000880: void fn0000000000000880()
// Called from:
//      _start
void fn0000000000000880()
{
	g_ptr12030();
}

// 00000000000008A0: void fn00000000000008A0()
// Called from:
//      _start
void fn00000000000008A0()
{
	g_ptr12040();
}

// 00000000000008B0: void fn00000000000008B0()
// Called from:
//      dumpline
void fn00000000000008B0()
{
	g_ptr12048();
}

// 00000000000008C0: void fn00000000000008C0()
// Called from:
//      hexdump
void fn00000000000008C0()
{
	g_ptr12050();
}

// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_text.h"

// 00000000000008E0: void _start()
void _start()
{
	fn0000000000000880();
	fn00000000000008A0();
	call_weak_fn();
}

// 0000000000000918: void call_weak_fn()
// Called from:
//      _init
//      _start
void call_weak_fn()
{
	if (g_qw11FC0 == 0x00)
		return;
	word64 x17_12;
	g_ptr12038();
}

// 0000000000000930: void deregister_tm_clones()
// Called from:
//      __do_global_dtors_aux
void deregister_tm_clones()
{
	if (0x00012077 - 0x00012070 <= 0x0E)
		return;
	<anonymous> * x1_13 = g_ptr11FB0;
	if (x1_13 == null)
		return;
	x1_13();
}

// 0000000000000968: void register_tm_clones()
// Called from:
//      frame_dummy
void register_tm_clones()
{
	int64 x1_7 = 0x00012070 - 0x00012070;
	if ((x1_7 >> 3) + ((x1_7 >> 3) >>u 63) >> 1 == 0x00)
		return;
	<anonymous> * x2_13 = g_ptr11FE0;
	if (x2_13 == null)
		return;
	word64 x2_17;
	x2_13();
}

// 00000000000009A8: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if ((word32) g_b12070 == 0x00)
	{
		if (g_qw11FB8 != 0x00)
			fn0000000000000840();
		deregister_tm_clones();
		g_b12070 = 0x01;
	}
}

// 00000000000009F0: void frame_dummy(Register word64 x29, Register word64 x30)
void frame_dummy(word64 x29, word64 x30)
{
	if (g_qw11DB8 != 0x00)
	{
		<anonymous> * x1_8 = g_ptr11FD8;
		if (x1_8 != null)
		{
			x1_8();
			register_tm_clones();
			return;
		}
	}
	register_tm_clones();
}

// 0000000000000A28: Register word64 dumpline(Register ptr64 x0, Register int32 w2)
// Called from:
//      hexdump
word64 dumpline(ptr64 x0, int32 w2)
{
	fn0000000000000850();
	int32 dwLoc6C_182 = w2;
	if (w2 > 0x10)
		dwLoc6C_182 = 0x10;
	int32 dwArg04_184 = 0x00;
	while (true)
	{
		int32 dwLoc44_198 = SEQ((word16) dwLoc42, wLoc44);
		if (dwArg04_184 >= dwLoc6C_182)
			break;
		fn0000000000000850();
		++dwArg04_184;
	}
	while (true)
	{
		++dwArg04_184;
		if (dwArg04_184 > 0x0F)
			break;
		fn0000000000000820();
		(word64) fp - 80 + ((word64) fp - 80) = (word32 *) g_dw0E40;
	}
	fn0000000000000820();
	(word64) fp - 80 + (word64) dwLoc44_198 = g_t0E48;
	word32 w0_80 = (word32) ((word64) fp - 80);
	int32 dwArg04_200 = 0x00;
	while (true)
	{
		Eq_161 dwLoc44_218 = SEQ((word16) dwLoc42, wLoc44);
		if (dwArg04_200 >= dwLoc6C_182)
			break;
		word32 w0_119;
		word32 w2_102 = w0_80 + 0x03 + dwArg04_200;
		if ((word32) (x0 + (word64) dwLoc42) > 0x1F && (word32) (x0 + (word64) dwLoc42) <= 0x7E)
			w0_119 = (word32) (x0 + (word64) dwLoc42);
		else
			w0_119 = 0x2E;
		(word64) fp - 80 + (int64) ((int32) w2_102) = (byte) w0_119;
		++dwArg04_200;
	}
	for (; dwArg04_200 <= 0x0F; ++dwArg04_200)
		(word64) fp - 80 + (int64) ((int32) (dwArg04_200 + (w0_80 + 0x03))) = 0x20;
	Mem162[fp + ~0x4F + (dwLoc44_218 + dwLoc42):word16] = (word16) (word32) Mem138[0x0000000000000E50<p64>:word16];
	fn00000000000008B0();
	return x30;
}

// 0000000000000C34: Register word64 hexdump(Register word64 x0)
// Called from:
//      main
word64 hexdump(word64 x0)
{
	word64 x0_120;
	struct Eq_224 * x29_34 = fp + -200;
	if ((word32) __stat(x0) != 0x00)
	{
		fn0000000000000830();
		x0_120 = SEQ(SLICE(x0, word32, 32), 0x01);
	}
	else
	{
		fn0000000000000870();
		if (x0 == 0x00)
		{
			fn0000000000000830();
			x0_120 = SEQ(SLICE(x0, word32, 32), 0x01);
		}
		else
		{
			while (x29_34->qw0058 - x29_34->qw00C8 > 0x00)
			{
				fn00000000000008C0();
				x29_34->dw00BC = (word32) ((char *) &x29_34->dw005E + 74);
				if (x29_34->dw00BC == 0x00)
					break;
				x29_34 = dumpline((char *) &x29_34->dw005E + 74, x29_34->dw00BC);
				x29_34->qw00C8 += (word64) x29_34->dw005E;
			}
			word32 x0_32_32_125 = x29_34->dw00C4;
			fn0000000000000860();
			x0_120 = SEQ(x0_32_32_125, 0x00);
		}
	}
	return x0_120;
}

// 0000000000000D18: void main(Register int32 w0, Register (arr word64) x1)
void main(int32 w0, word64 x1[])
{
	int32 dwArg04_60;
	int32 dwLoc12_75 = SEQ(wLoc10, SLICE(x1, word16, 48));
	for (dwArg04_60 = 0x01; dwArg04_60 < w0; ++dwArg04_60)
		hexdump(x1[(word64) dwLoc12_75]);
}

// 0000000000000D88: void __libc_csu_init(Register word32 w0, Register word64 x1, Register word64 x2, Register word64 x24)
void __libc_csu_init(word32 w0, word64 x1, word64 x2, word64 x24)
{
	word32 x24_32_32_104 = SLICE(x24, word32, 32);
	int64 x20_26 = 0x00011DB0 - 0x00011DA8;
	<anonymous> * x21_24[] = g_a11DA8;
	word64 x22_34 = x2;
	word64 x23_38 = x1;
	word64 x30_44;
	word64 x29_43 = _init(out x30_44);
	int64 x20_45 = x20_26 >> 3;
	if (x20_26 >> 3 != 0x00)
	{
		int64 x19_46 = 0x00;
		word64 x24_151 = SEQ(x24_32_32_104, w0);
		do
		{
			word64 x3_76;
			x21_24[x19_46]();
			int64 x31_81 = x20_45 - x19_46;
		} while (x31_81 != 0x00);
	}
}

// 0000000000000E00: void __libc_csu_fini()
void __libc_csu_fini()
{
}

// 0000000000000E08: Register word64 __stat(Register word64 x0)
// Called from:
//      hexdump
word64 __stat(word64 x0)
{
	word32 x0_32_32_12 = SLICE(x0, word32, 32);
	word64 x0_13 = SEQ(x0_32_32_12, 0x00);
	word64 x0_17;
	g_ptr12058();
	return x0_17;
}

