...

{
  ...
  for (c = 0; c < cnt; c = c + 1) {
    sprintf(buff + c * 3 + 9," %02lX",(uint)p[c]);
  }
  while (bVar2 = c < 0x10, c = c + 1, bVar2) {
    strcat(buff,"   ");
  }
  sVar4 = strlen(buff);
  memcpy(buff + sVar4,&DAT_08048a32,4);
  iVar1 = sVar4 + 3;
  for (c = 0; c < cnt; c = c + 1) {
  ...
  memcpy(buff + iVar1 + c,&DAT_08048a36,2);
  puts(buff);
...

{
  ...
  if (iVar2 == 0) {
    ...
    if (__stream == (FILE *)0x0) {
      ...
      offset = 0;
      while ((offset < (uint)st.st_size && (cnt_00 = fread(buff,1,0x10,__stream), cnt_00 != 0))) {
        dumpline(buff,offset,cnt_00);
        offset = offset + cnt_00;
      }
      fclose(__stream);
      iVar2 = 0;
    }
...

{
  ...
  local_18 = 0;
  for (local_14 = 1; local_14 < argc; local_14 = local_14 + 1) {
    iVar1 = hexdump(argv[local_14]);
...
// WARNING: Removing unreachable block (ram,0x0804895e)
// WARNING: Removing unreachable block (ram,0x08048960)
