...

{
  ...
  c = 0;
  while (c < local_74) {
    sprintf(buff + (longlong)(c * 3) + 9," %02lX",(ulonglong)p[c]);
    c = c + 1;
  }
  while (c < 0x10) {
    sVar3 = strlen(buff);
  ...
  *(undefined2 *)(buff + (longlong)len + (longlong)c) = 0x7c;
  puts(buff);
  return;
}
...

{
  ...
  if (iVar1 == 0) {
    ...
    if (__stream == (FILE *)0x0) {
      ...
      offset = 0;
      while ((offset < (ulonglong)st.st_size && (cnt_00 = fread(buff,1,0x10,__stream), cnt_00 != 0))
            ) {
        dumpline(buff,offset,cnt_00);
        offset = offset + (longlong)(int)cnt_00;
      }
      fclose(__stream);
      iVar1 = 0;
    }
