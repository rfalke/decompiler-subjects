...

{
  ...
  for (; (int)lVar6 < cnt; lVar6 = lVar6 + 1) {
    sprintf(pcVar4," %02lX",(ulong)p[lVar6]);
    pcVar4 = pcVar4 + 3;
  }
  iVar2 = cnt;
  iVar1 = cnt;
  ...
  sVar3 = strlen(buff);
  iVar2 = (int)sVar3;
  pcVar4 = strcpy(buff + iVar2,"  |");
  for (lVar6 = 0; (int)lVar6 < cnt; lVar6 = lVar6 + 1) {
    uVar5 = p[lVar6];
  ...
  lVar6 = (long)iVar1;
  lVar7 = (iVar2 + iVar1) - lVar6;
  for (; (int)lVar6 < 0x10; lVar6 = lVar6 + 1) {
    buff[lVar6 + lVar7] = ' ';
  }
  strcpy(buff + (long)iVar2 + (long)(int)lVar6,"|");
  puts(buff);
  return;
}
...

{
  ...
  if ((iVar1 == 0) || (__stream = fopen(fname,"rb"), __stream == (FILE *)0x0)) {
    ...
    for (offset = 0; offset < (ulong)st.st_size; offset = offset + (long)iVar1) {
      sVar2 = fread(buff,1,0x10,__stream);
      iVar1 = (int)sVar2;
      if (iVar1 == 0) break;
      dumpline(buff,offset,iVar1);
    }
    fclose(__stream);
    iVar1 = 0;
  }
...

{
  ...
  UNRECOVERED_JUMPTABLE = (code *)UndefinedInstructionException(0,0x100d0c);
