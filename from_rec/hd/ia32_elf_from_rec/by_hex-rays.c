...
void fini_dummy();
int __cdecl dumpline(int a1, int a2, signed int a3);
...
int environ = 0; // weak
int force_to_data = 4294967295; // weak
...
void __noreturn __crt_dummy__()
{
  ...
  atexit(term_proc);
  init_proc();
  v1 = main(v2, (const char **)&retaddr, (const char **)&retaddr + v2 + 1);
  exit(v1);
}
...
int __cdecl dumpline(int a1, int a2, signed int a3)
{
  ...
  signed int i; // [esp+8h] [ebp-54h]
  signed int j; // [esp+8h] [ebp-54h]
  char s[80]; // [esp+Ch] [ebp-50h]
...
{
  int result; // eax
  signed int v2; // [esp+0h] [ebp-5Ch]
  char v3; // [esp+4h] [ebp-58h]
  unsigned int v4; // [esp+18h] [ebp-44h]
  FILE *stream; // [esp+44h] [ebp-18h]
  unsigned int i; // [esp+48h] [ebp-14h]
  char ptr; // [esp+4Ch] [ebp-10h]

  if ( stat((int)s, (int)&v3) )
  {
    stream = fopen(s, "rb");
    if ( stream )
    {
