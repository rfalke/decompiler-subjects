...
??? _start(???)
{
  ...
  (void) STORE(local + 24, 134513524);
  (void) STORE(local + 28, 134513572);
  ...
  (void) F_080482a6();
  reg_result = LOAD(LOAD(local) + 4822);
  if(reg_result  !=  0) {
...
{
  (void) F_080482a6();
  reg_result = LOAD(InitValue(initial_value_of_eax) + 4822);
  if(reg_result  !=  0) {
    reg_result = intern_redirectCall(reg_result);
  }
  return reg_result;
...
??? F_080482a6(???)
{
  reg_result = LOAD(LOAD(InitStackPointer) + 4822);
  if(reg_result  !=  0) {
    reg_result = intern_redirectCall(reg_result);
  }
  return reg_result;
...
??? __libc_csu_init(???)
{
  ...
  if(reg_var1  <  reg_result) {
    do {
      reg_result = intern_redirectCall(LOAD(reg_var1 * 4 + 134517888));
      reg_var1++;
...
// addr = 08048406.0
// signature= func(F_08048406, ret=[<0, int(undef, 4),null,reg[eax]>], para=[], varargs=false)
??? F_08048406(???)
{
  reg_result = __do_global_dtors_aux();
  return reg_result;
