// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_fini.h"

// 080485F4: void _fini()
// Called from:
//      __libc_csu_fini
void _fini()
{
	__do_global_dtors_aux();
}

// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_init.h"

// 08048278: Register word32 _init(Register word32 edx)
// Called from:
//      __libc_csu_init
word32 _init(word32 edx)
{
	call_gmon_start();
	frame_dummy();
	return __do_global_ctors_aux(edx);
}

// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_plt.h"

// 080482A0: void puts()
void puts()
{
}

// 080482B0: void malloc()
void malloc()
{
}

// 080482C0: void strlen()
void strlen()
{
}

// 080482D0: void __libc_start_main()
void __libc_start_main()
{
}

// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject_text.h"

// 080482E0: void _start(Register (ptr32 Eq_13) edx, Stack int32 dwArg00)
void _start(void (* edx)(), int32 dwArg00)
{
	__align((char *) fp + 4);
	__libc_start_main(&g_t8048390, dwArg00, (char *) fp + 4, &g_t804856C, &g_t804859C, edx, fp);
	__hlt();
}

// 08048304: void call_gmon_start()
// Called from:
//      _init
void call_gmon_start()
{
	<anonymous> * eax_16 = g_ptr804AD88;
	if (eax_16 != null)
		eax_16();
}

// 08048328: void __do_global_dtors_aux()
// Called from:
//      _fini
void __do_global_dtors_aux()
{
	if (g_b804AD8C == 0x00)
	{
		<anonymous> ** eax_11 = g_ptr804AB28;
		<anonymous> * edx_12 = *eax_11;
		while (edx_12 != null)
		{
			<anonymous> ** eax_19 = (char *) eax_11 + 4;
			g_ptr804AB28 = eax_19;
			edx_12();
			eax_11 = g_ptr804AB28;
			edx_12 = (<anonymous> *) *eax_11;
		}
		g_b804AD8C = 0x01;
	}
}

// 08048364: void frame_dummy()
// Called from:
//      _init
void frame_dummy()
{
	if (g_dw804AD68 != 0x00 && 0x00 != 0x00)
		fn00000000();
}

// 08048390: void main()
void main()
{
	__align(fp - 0x9C);
	struct Eq_86 * eax_14 = malloc(0x0C);
	eax_14->dw0004 = 134519545;
	word32 dwLoc10_192 = 0x02;
	int32 * dwLoc14_193 = &eax_14->dw0004;
	while (true)
	{
		word32 v7_28 = dwLoc10_192 - 0x01;
		dwLoc10_192 = v7_28;
		if (v7_28 == 0x00)
			break;
		Eq_89 eax_38 = strlen(*dwLoc14_193);
		Eq_89 dwLoc24_198 = eax_38;
		if (eax_38 > 0x0A)
			dwLoc24_198.u0 = 0x0A;
		int32 dwLoc18_200;
		for (dwLoc18_200 = 0x00; dwLoc18_200 <= 0x06; ++dwLoc18_200)
		{
			Eq_89 dwLoc1C_202 = 0x00;
			while (dwLoc1C_202 < dwLoc24_198)
			{
				int32 eax_59 = (int32) *((word32) dwLoc1C_202 + *dwLoc14_193);
				int32 dwLoc28_206 = eax_59 - 0x20;
				if (eax_59 < 0x20)
					dwLoc28_206 = 0x00;
				Eq_158 dwLoc20_208 = 0x00;
				while (dwLoc20_208 <= 0x06)
				{
					struct Eq_163 * eax_77 = (word32) dwLoc20_208 + ((dwLoc1C_202 << 0x03) + (fp - 0x0C));
					int32 dwLoc84_213 = dwLoc28_206;
					if (dwLoc28_206 < 0x00)
						dwLoc84_213 = dwLoc28_206 + 0x07;
					int32 edx_95 = dwLoc18_200 + (((dwLoc84_213 >> 0x03) << 0x03) - (dwLoc84_213 >> 0x03));
					int32 dwLoc90_221 = dwLoc28_206;
					if (dwLoc28_206 < 0x00)
						dwLoc90_221 = dwLoc28_206 + 0x07;
					Eq_195 edx_112 = dwLoc28_206 - ((dwLoc90_221 >> 0x03) << 0x03);
					Mem122[eax_77 + -112:byte] = Mem107[(edx_112 << 0x03) - edx_112 + dwLoc20_208 + 0x0804AB40[edx_95 * 0x04]:byte];
					dwLoc20_208 = (word32) dwLoc20_208 + 1;
				}
				(fp - 117)[dwLoc1C_202].b0000 = 0x20;
				dwLoc1C_202 = (word32) dwLoc1C_202 + 1;
			}
			int32 dwLoc1C_232 = (dwLoc24_198 << 0x03) - 0x01;
			while (dwLoc1C_232 >= 0x00 && (&((&((&((&((&((&((&((&((&((&((&((&((&((&((&((&((&((&((&((fp - 0x7C))[dwLoc1C_232].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000))[0].a0000)[0] == 0x20)
			{
				(&(&(&(&(&(&(&(&(&(&(&(&(&(&(&(&(&(&(&(fp - 0x7C)[dwLoc1C_232].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0].a0000)[0] = (Eq_220) 0x00;
				--dwLoc1C_232;
			}
			puts(fp - 0x7C);
		}
		puts("");
		++dwLoc14_193;
	}
}

// 0804856C: void __libc_csu_init(Register word32 edx)
void __libc_csu_init(word32 edx)
{
	word32 edx_16 = _init(edx);
	uint32 ebx_24 = 0x00;
	if (false)
	{
		do
		{
			((<anonymous> *[]) 0x0804AB0C)[ebx_24]();
			++ebx_24;
		} while (ebx_24 < 0x00);
	}
}

// 0804859C: void __libc_csu_fini()
void __libc_csu_fini()
{
	int32 eax_16 = 0;
	ui32 ebx_19 = ~0x00;
	if (false)
	{
		do
		{
			((<anonymous> *[]) 0x0804AB0C)[ebx_55]();
			ebx_19 = ebx_55 - 0x01;
			ebx_55 = ebx_19;
		} while (ebx_55 != 0x00);
	}
	_fini();
}

// 080485D0: Register word32 __do_global_ctors_aux(Register word32 edx)
// Called from:
//      _init
word32 __do_global_ctors_aux(word32 edx)
{
	<anonymous> * eax_14 = g_ptr804AD58;
	word32 * ebx_16 = &g_ptr804AD58;
	while (eax_14 != (<anonymous> *) ~0x00)
	{
		eax_14();
		ebx_16 -= 0x04;
		eax_14 = *ebx_16;
	}
	return edx;
}

