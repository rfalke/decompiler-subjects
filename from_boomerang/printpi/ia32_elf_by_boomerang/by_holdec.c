...
??? _start(???)
{
  ...
  (void) STORE(local + 24, 134513504);
  (void) STORE(local + 28, 134513552);
  ...
  (void) F_080482a6();
  reg_result = LOAD(LOAD(local) + 4730);
  if(reg_result  !=  0) {
...
{
  (void) F_080482a6();
  reg_result = LOAD(InitValue(initial_value_of_eax) + 4730);
  if(reg_result  !=  0) {
    reg_result = intern_redirectCall(reg_result);
  }
  return reg_result;
...
??? __libc_csu_init(???)
{
  ...
  if(reg_var1  <  reg_result) {
    do {
      reg_result = intern_redirectCall(LOAD(reg_var1 * 4 + 134517796));
      reg_var1++;
