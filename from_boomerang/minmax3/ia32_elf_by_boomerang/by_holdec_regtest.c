...
void test(s4 n)
{
  reg_aa = CONSTRUCT(NAMED(newC, BIT_TEST(30, (n * -1 + 0xfffffffe & BIT_TO_INT(n < 0) + BIT_TO_INT(n < 0 ? n > -2 : n < 0) * -1 + 0xffffffff) * -1 + -2)), NAMED(newO, UNDEF), NAMED(newV, (n * -1 + 0xfffffffe & BIT_TO_INT(n < 0) + BIT_TO_INT(n < 0 ? n > -2 : n < 0) * -1 + 0xffffffff) > -2 ? -1 : 0));
