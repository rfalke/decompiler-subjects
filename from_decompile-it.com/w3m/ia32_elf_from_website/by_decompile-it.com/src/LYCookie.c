...
static void MemAllocCopy( char **dest, char *start, char *end );
static cookie *newCookie( void );
static void freeCookie( cookie *co );
static BOOLEAN host_matches( char *A, char *B );
static BOOLEAN port_matches( int port, char *list );
static int ignore_trailing_slash( char *a );
static BOOLEAN is_prefix( char *a, char *b );
static domain_entry *find_domain_entry( char *name );
static void store_cookie( cookie *co, char *hostname, char *path );
static char *scan_cookie_sublist( char *hostname, char *path, int port, HTList *sublist, char *header, BOOLEAN secure );
static char *alloc_attr_value( char *value_start, char *value_end );
static unsigned int parse_attribute( unsigned int flags, cookie *cur_cookie, int *cookie_len, char *attr_start, int attr_len, char *value, char *address, char *hostname, int port );
static void LYProcessSetCookies( char *SetCookie, char *SetCookie2, char *address, char *hostname, char *path, int port );
static FILE *NewCookieFile( char *cookie_file );
static int LYHandleCookies( char *arg, HTParentAnchor *anAnchor, HTFormat format_out, HTStream *sink );
static void cookie_domain_flag_set( char *domainstr, int flag );
static HTList *domain_list;
static HTList *cookie_list;
static int total_cookies;
static int number_of_file_cookies;
HTProtocol LYLynxCookies = { "LYNXCOOKIE", &LYHandleCookies, 0,  }
;
...
BOOLEAN host_matches( char *A, char *B )
{
  ...
  {
    if ( B[0] == '.' && ( B[1] & 255 ) != 0 && B[1] != '.' && A[0] != '.' )
    {
      int diff = strlen( A ) - strlen( B );
      if ( diff >= 1 && strcasecomp( &A[ diff ], B ) == 0 )
      {
      }
    }
  }
...
BOOLEAN port_matches( int port, char *list )
{
  ...
  do
  {
    if ( ( number[0] & 255 ) != 0 )
    {
      ...
      {
        for ( ; ( *(short*)(*(int*)(__ctype_b_loc( )) + ( number[0] * 2 )) & 2048 ) != 0; number++ )
        {
          //number++;
        }
        for ( ; ( number[0] & 255 ) != 0 && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( number[0] * 2 )) & 2048 ) == 0; number++ )
        }
      }
      else
      {
    ...
    while ( ( number[0] & 255 ) == 0 || ( *(short*)(*(int*)(__ctype_b_loc( )) + ( number[0] * 2 )) & 2048 ) != 0 );
    number++;
    return 1;
}
...
BOOLEAN is_prefix( char *a, char *b )
{
  ...
  else
  {
    ...
    if ( len_a < len_b && ( len_a > 1 || a[0] != '/' ) && ( b[ len_a ] & 255 ) != 0 && b[ len_a ] != '/' )
    {
    }
    else
    {
...
domain_entry *find_domain_entry( char *name )
{
  ...
  if ( name != 0 && ( name[0] & 255 ) != 0 )
  {
    ...
    for ( ; hl != 0; hl = hl->next )
    {
      de = &hl->object[0]/*error:'v'*/;
      if ( de != 0 && de->domain != 0 )
      {
        if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
        {
          fprintf( TraceFP( ), "...test_domain_entry(%s) bv:%u, invcheck_bv:%u\n", de->domain, de->bv, de->invcheck_bv );
        }
        if ( strcasecomp( name, de->domain ) == 0 )
          break;
      }
      de = 0;
      //hl = hl->next;
...
void store_cookie( cookie *co, char *hostname, char *path )
{
  ...
  if ( co != 0 )
  {
    ...
    {
      invcheck_behaviour_t invcheck_bv = de == 0 ? INVCHECK_QUERY : de->invcheck_bv;
      switch ( invcheck_bv )
      {
      default:
        break;
    ...
    if ( strcasecomp( co->domain, hostname ) != 0 )
    {
      ...
      {
        if ( co->domain[0] != '.' || ( co->domain[1] & 255 ) == 0 )
        {
        ...
        {
          ptr = strchr( &co->domain[1], 46 );
          if ( ptr == 0 || ( ptr[1] & 255 ) == 0 )
          {
            if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
            {
              fprintf( TraceFP( ), "store_cookie: Rejecting domain '%s'.\n", co->domain );
    ...
    for ( ; hl != 0; hl = next )
    {
      c2 = &hl->object[0]/*error:'v'*/;
      next = hl->next;
      if ( c2 != 0 && ( c2->flags & 4 ) != 0 && c2->expires <= now )
      {
      ...
      if ( c2 != 0 && strcasecomp( co->domain, c2->domain ) == 0 && strcmp( co->path, c2->path ) == 0 && strcmp( co->name, c2->name ) == 0 )
      {
        HTList_removeObject( cookie_list, (void*)c2 );
        freeCookie( c2 );
        c2 = 0;
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            if ( Replacement == 1 && de != 0 && de->bv != REJECT_ALWAYS )
            {
            ...
            {
              if ( ( co->flags & 32 ) != 0 || ( HTConfirmCookie( de, hostname, co->name, co->value ) & 255 ) != 0 )
              {
                HTList_insertObjectAt( cookie_list, (void*)co, pos );
                total_cookies++;
              }
...
{
  char *value = 0;
  if ( value_start != 0 && value_start <= value_end )
  {
...
unsigned int parse_attribute( unsigned int flags, cookie *cur_cookie, int *cookie_len, char *attr_start, int attr_len, char *value, char *address, char *hostname, int port )
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        known_attr = 1;
        if ( cur_cookie != 0 && value != 0 && cur_cookie->comment == 0 )
        {
      ...
      else
      {
        ...
        {
          known_attr = 1;
          if ( cur_cookie != 0 && value != 0 && cur_cookie->commentURL == 0 )
          {
            cur_cookie->commentURL = HTParse( value, address, 31 );
            url_type = is_url( cur_cookie->commentURL );
            if ( url_type != 0 && ( url_type == 2 || url_type == 20 ) )
            {
        ...
        else
        {
          ...
          {
            known_attr = 1;
            if ( cur_cookie != 0 && value != 0 && ( cur_cookie->flags & 8 ) == 0 )
            {
              cookie_len[0] -= strlen( cur_cookie->domain );
              if ( value[0] != '.' && ( value[0] & 255 ) != 0 && ( value[1] & 255 ) != 0 && strcasecomp( value, hostname ) != 0 )
              {
                char *ptr = strchr( value, 46 );
                if ( ptr != 0 && ( ptr[1] & 255 ) != 0 )
                {
                  ptr = value;
                  for ( ; ptr[0] == '.' || ( *(short*)(*(int*)(__ctype_b_loc( )) + ( ptr[0] * 2 )) & 2048 ) != 0; ptr++ )
                  {
                    //ptr++;
                  }
                  if ( ( ptr[0] & 255 ) != 0 )
          ...
          else
          {
            ...
            {
              known_attr = 1;
              if ( cur_cookie != 0 && value != 0 && ( cur_cookie->flags & 16 ) == 0 )
              {
            ...
            else
            {
              ...
              {
                if ( cur_cookie != 0 && value != 0 && cur_cookie->PortList == 0 )
                {
                  char *cp = value;
                  for ( ; ( cp[0] & 255 ) != 0 && ( ( *(short*)(*(int*)(__ctype_b_loc( )) + ( cp[0] * 2 )) & 2048 ) != 0 || cp[0] == ',' || cp[0] == ' ' ); cp++ )
                  {
                    //cp++;
                  }
                  if ( ( cp[0] & 255 ) == 0 )
              ...
              else
              {
                ...
                {
                  known_attr = 1;
                  if ( cur_cookie != 0 && value != 0 && cur_cookie->version < 1 )
                  {
                ...
                else
                {
                  ...
                  {
                    known_attr = 1;
                    if ( cur_cookie != 0 && value != 0 && ( flags & 4 ) == 0 )
                    {
                      int temp = strtol( value, 0, 10 );
                      cur_cookie->flags |= 4;
                      if ( *(int*)(__errno_location( )) == -34 )
                  ...
                  else
                  {
                    ...
                    {
                      known_attr = 1;
                      if ( cur_cookie != 0 && ( flags & 4 ) == 0 && ( cur_cookie->flags & 4 ) == 0 && value != 0 )
                      {
                        cur_cookie->flags |= 4;
                        cur_cookie->expires = LYmktime( value, 0 );
                        if ( cur_cookie->expires >= 1 && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
                        {
                          fprintf( TraceFP( ), "LYSetCookie: expires %d, %s", cur_cookie->expires, ctime( &cur_cookie->expires ) );
                        }
                      }
...
void LYProcessSetCookies( char *SetCookie, char *SetCookie2, char *address, char *hostname, char *path, int port )
{
  ...
  if ( ( SetCookie != 0 && ( SetCookie[0] & 255 ) != 0 ) || ( SetCookie2 != 0 && ( SetCookie2[0] & 255 ) != 0 ) )
  {
    CombinedCookies = HTList_new( );
    p = SetCookie2 == 0 ? "" : SetCookie2;
    if ( SetCookie != 0 && ( p[0] & 255 ) != 0 && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
    {
      fprintf( TraceFP( ), "LYProcessSetCookies: Using Set-Cookie2 header.\n" );
      while ( NumCookies <= max_cookies_domain && ( p[0] & 255 ) != 0 )
      {
        attr_start = attr_end = value_start = value_end = 0;
        p = LYSkipCBlanks( p );
        attr_start = p;
        for ( ; ( p[0] & 255 ) != 0 && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( p[0] * 2 )) & 8192 ) == 0 && p[0] != '=' && p[0] != ';' && p[0] != ','; p++ )
        {
          //p++;
        }
        attr_end = p;
        ...
        if ( p[0] == '=' || strncasecomp( attr_start, "Expires", 7 ) == 0 )
        {
          ...
          if ( attr_end - attr_start == 7 && strncasecomp( attr_start, "Expires", 7 ) == 0 )
          {
            ...
            {
              spaces--;
              while ( ( p[0] & 255 ) != 0 && p[0] != ';' && p[0] != ',' && spaces != 0 )
              {
                p++;
                if ( ( *(short*)(*(int*)(__ctype_b_loc( )) + ( p[0] * 2 )) & 8192 ) != 0 )
                {
                  for ( ; ( *(short*)(*(int*)(__ctype_b_loc( )) + ( p[1] * 2 )) & 8192 ) != 0; p++ )
                  {
                    //p++;
                  }
                  spaces--;
            ...
            else
            {
              ...
              while ( p[0] == ',' || ( *(short*)(*(int*)(__ctype_b_loc( )) + ( p[0] * 2 )) & 8192 ) != 0 )
              {
                p++;
              }
              spaces--;
          ...
          {
            if ( attr_end - attr_start == 4 && strncasecomp( attr_start, "port", 4 ) == 0 && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( p[0] * 2 )) & 2048 ) != 0 )
            {
              char *cp, *cp1;
              value_start = p;
              ...
              if ( ( p[0] & 255 ) != 0 && p[0] != ';' )
              {
                if ( p[0] == ',' )
                {
                  ...
                  {
                    cp1 = cp;
                    for ( ; ( *(short*)(*(int*)(__ctype_b_loc( )) + ( cp1[0] * 2 )) & 2048 ) != 0; cp1++ )
                    {
                      //cp1++;
                    }
                    cp1 = LYSkipCBlanks( cp1 );
                    if ( ( cp1[0] & 255 ) == 0 || cp1[0] == ',' || cp1[0] == ';' )
                    {
                      p = cp;
                    }
                    else
                    {
                      while ( ( p[0] & 255 ) != 0 && p[0] != ';' && p[0] != ',' )
                      {
                        p++;
                      }
                      value_end = p;
                      if ( value_start < value_end && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( value_end[ -1 ] * 2 )) & 8192 ) != 0 )
                      {
                        value_end = &value_end[ -1 ];
                        for ( ; value_start[1] < value_end && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( value_end[0] * 2 )) & 8192 ) != 0; value_end = &value_end[ -1 ] )
                        {
                          if ( ( *(short*)(*(int*)(__ctype_b_loc( )) + ( value_end[ -1 ] * 2 )) & 8192 ) == 0 )
                          {
                            break;
                          }
                          else
            ...
            else
            {
              ...
              else
              {
                ...
                {
                  value_end = &value_end[ -1 ];
                  for ( ; value_start[1] < value_end && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( value_end[0] * 2 )) & 8192 ) != 0 && ( *(short*)(*(int*)(__ctype_b_loc( )) + ( value_end[ -1 ] * 2 )) & 8192 ) != 0; value_end = &value_end[ -1 ] )
                  {
                    //value_end = &value_end[ -1 ];
                  }
                }
        ...
        else
        {
          ...
          if ( ( parse_flags & 2 ) == 0 && value != 0 && value_start <= value_end )
          {
            if ( cookie_len <= max_cookies_buffer && cur_cookie != 0 && ( parse_flags & 1 ) == 0 )
            {
            ...
            else
            {
              if ( cur_cookie != 0 )
              {
                if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
                {
                  fprintf( TraceFP( ), "LYProcessSetCookies: Rejecting Set-Cookie2: %s=%s\n", cur_cookie->name == 0 ? "[no name]" : cur_cookie->name, cur_cookie->value == 0 ? "[no value]" : cur_cookie->value );
                }
                if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
                {
                  fprintf( TraceFP( ), ( parse_flags & 1 ) == 0 ? "                     due to invalid port!\n" : "                     due to excessive length!\n" );
                }
                if ( ( parse_flags & 1 ) != 0 )
                  NumCookies--;
      ...
      if ( NumCookies <= max_cookies_domain && cookie_len <= max_cookies_buffer && cur_cookie != 0 && ( parse_flags & 1 ) == 0 )
      {
        if ( cur_cookie->version < 1 )
          cur_cookie->version = 1;
        HTList_appendObject( CombinedCookies, (void*)cur_cookie );
      ...
      {
        if ( cur_cookie != 0 && ( parse_flags & 1 ) == 0 )
        {
          if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
          {
            fprintf( TraceFP( ), "LYProcessSetCookies: Rejecting Set-Cookie2: %s=%s\n", cur_cookie->name == 0 ? "[no name]" : cur_cookie->name, cur_cookie->value == 0 ? "[no value]" : cur_cookie->value );
          ...
          if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
          {
            fprintf( TraceFP( ), "                     due to excessive %s%s%s\n", cookie_len <= max_cookies_buffer ? "" : "length", cookie_len <= max_cookies_buffer || NumCookies <= max_cookies_domain ? "" : " and ", NumCookies <= max_cookies_domain ? "!\n" : "number!\n" );
          }
          freeCookie( cur_cookie );
          cur_cookie = 0;
        }
        ...
        NumCookies = 0;
        cur_cookie = 0;
        p = SetCookie == 0 || ( SetCookie2 != 0 && ( SetCookie2[0] & 255 ) != 0 ) ? "" : SetCookie;
        if ( SetCookie2 != 0 && ( p[0] & 255 ) != 0 && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
        {
          ...
          while ( NumCookies <= max_cookies_domain && ( p[0] & 255 ) != 0 )
          {
            ...
            else
            {
              ...
              if ( ( parse_flags & 2 ) == 0 && value != 0 && value_start <= value_end )
              {
                if ( cookie_len <= max_cookies_buffer && cur_cookie != 0 )
                {
                  if ( SetCookie2 != 0 )
                  {
                    if ( cur_cookie->version < 1 )
          ...
          if ( NumCookies <= max_cookies_domain && cookie_len <= max_cookies_buffer && cur_cookie != 0 )
          {
            if ( SetCookie2 != 0 )
            {
              if ( cur_cookie->version < 1 )
          ...
          else
          {
            ...
            while ( 1 )
            {
              ...
              {
                cl = cl->next;
                co = cl == 0 ? 0 : &cl->object[0]/*error:'v'*/;
                if ( co != 0 )
                {
                  if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
                  {
                    fprintf( TraceFP( ), "LYProcessSetCookie: attr=value pair: '%s=%s'\n", co->name == 0 ? "[no name]" : co->name, co->value == 0 ? "[no value]" : co->value );
                  }
                  if ( co->expires >= 1 && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
                  {
...
void LYSetCookie( char *SetCookie, char *SetCookie2, char *address )
{
  ...
  if ( hostname != 0 )
  {
    ...
    if ( ptr != 0 )
    {
      ...
      if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
      {
        fprintf( TraceFP( ), "LYSetCookie called with host '%s', path '%s',\n", hostname == 0 ? "" : hostname, path == 0 ? "" : path );
      }
      if ( SetCookie != 0 && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
      {
        fprintf( TraceFP( ), "    and Set-Cookie: '%s'\n", SetCookie );
      }
      if ( SetCookie2 != 0 && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
      {
        fprintf( TraceFP( ), "    and Set-Cookie2: '%s'\n", SetCookie2 );
      }
      if ( ( ( LYSetCookies & 255 ) == 0 || BadHeaders == 1 ) && ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
      {
        fprintf( TraceFP( ), "    Ignoring this Set-Cookie/Set-Cookie2 request.\n" );
      }
      if ( ( LYSetCookies & 255 ) == 0 || BadHeaders == 1 )
      {
        if ( hostname != 0 )
        {
          free( hostname );
...
char *LYAddCookieHeader( char *hostname, char *path, int port, BOOLEAN secure )
{
  ...
  if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
  {
    fprintf( TraceFP( ), "LYCookie: Searching for '%s:%d', '%s'.\n", hostname == 0 ? "(null)" : hostname, port, path == 0 ? "(null)" : path );
  }
  while ( hl != 0 )
  {
    de = &hl->object[0]/*error:'v'*/;
    ...
    if ( de != 0 )
    {
      ...
      if ( de->bv == QUERY_USER && de->invcheck_bv == INVCHECK_QUERY )
      {
        if ( de->domain != 0 )
        {
          free( de->domain );
  ...
  if ( cookie_handle != 0 )
  {
    ...
    while ( LYSafeGets( &buf, cookie_handle ) == 0 )
    {
      ...
      else
      {
        ...
        tok_loop = 0;
        for ( ; tok_out != 0 && tok_values[ tok_loop ].s != 0; tok_loop++ )
        {
          if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
          {
        ...
        else
        {
          ...
          if ( value[0] == '"' && ( value[1] & 255 ) != 0 && value[ strlen( value ) - 1 ] == '"' && value[ strlen( value ) - 2 ] != '\\' )
          {
...
void LYStoreCookies( char *cookie_file )
{
  ...
  time_t now = time( 0 );
  if ( cookie_file != 0 && ( cookie_file[0] & 255 ) != 0 && strcmp( cookie_file, "/dev/null" ) != 0 && ( ( domain_list != 0 && domain_list->next != 0 ) || number_of_file_cookies != 0 ) )
  {
...
B10:    dl = domain_list;
    for ( ; dl != 0; dl = dl->next )
    {
      de = &dl->object[0]/*error:'v'*/;
      if ( de != 0 )
      {
        cl = de->cookie_list;
        for ( ; cl != 0; cl = cl->next )
        {
          co = &cl->object[0]/*error:'v'*/;
          if ( co != 0 )
          {
            if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
            {
...
B42:                  fprintf( cookie_handle, "%s\t%s\t%s\t%s\t%d\t%s\t%s%s%s\n", de->domain, de->domain[0] == '.' ? "TRUE" : "FALSE", co->path, ( co->flags & 1 ) == 0 ? "FALSE" : "TRUE", co->expires, co->name, ( co->quoted & 255 ) == 0 ? "" : "\"", co->value == 0 ? "" : co->value, ( co->quoted & 255 ) == 0 ? "" : "\"" );
                  if ( ( WWW_TraceFlag & 255 ) != 0 && ( WWW_TraceMask & 32 ) != 0 )
                  {
...
int LYHandleCookies( char *arg, HTParentAnchor *anAnchor, HTFormat format_out, HTStream *sink )
{
  ...
  if ( domain != 0 )
  {
    ...
    else
    {
      ...
      if ( lynxID != 0 && ( lynxID[0] & 255 ) == 0 && lynxID != 0 )
      {
        free( lynxID );
        lynxID = 0;
      }
  ...
  if ( domain != 0 )
  {
    ...
    if ( de != 0 )
    {
      ...
      if ( lynxID != 0 )
      {
        ...
        {
          co = &cl->object[0]/*error:'v'*/;
          if ( co != 0 && strcmp( lynxID, co->lynxID ) == 0 )
          {
            ...
            else
            {
              ...
              co = 0;
              total_cookies--;
              if ( de->bv == QUERY_USER && ( de->cookie_list == 0 || de->cookie_list->next == 0 ) && ( HTConfirm( gettext( "Delete this empty domain?" ) ) & 255 ) != 0 )
              {
                if ( de->domain != 0 )
                {
                  free( de->domain );
        ...
        while ( 1 )
        {
          ...
          case 68:
            ...
            else
            {
              ...
              if ( ( HTConfirm( gettext( "Delete this empty domain?" ) ) & 255 ) != 0 )
              {
                if ( de->domain != 0 )
                {
                  free( de->domain );
          ...
          case 80:
            de->bv = QUERY_USER;
            HTUserMsg2( gettext( "'P'rompting to allow from domain '%s'." ), de->domain );
            return -204;
            break;
          case 86:
            de->bv = REJECT_ALWAYS;
          ...
          default:
            ...
            for ( ; cl != 0; cl = cl->next )
            {
              ...
              if ( co != 0 )
              {
                ...
                HTSprintf0( &buf, "&lt;dd&gt;&lt;a href=\"%s//%s/%s\"&gt;%s=%s&lt;/a&gt;\n", "LYNXCOOKIE:", de->domain, co->lynxID, name, value );
                if ( name != 0 )
                {
                  free( name );
                  name = 0;
                ...
                HTSACopy( &path, "/" );
                HTSprintf0( &buf, "&lt;dd&gt;Path=%s\n&lt;dd&gt;Port: %d Secure: %s Discard: %s\n", path, co->port, ( co->flags & 1 ) == 0 ? "NO" : "YES", ( co->flags & 2 ) == 0 ? "NO" : "YES" );
                if ( path != 0 )
                {
                  free( path );
                  path = 0;
                ...
                HTSprintf0( &buf, "&lt;dd&gt;&lt;em&gt;%s&lt;/em&gt; %s%s", gettext( "Maximum Gobble Date:" ), gettext( "(End of session.)" ), ( co->flags & 4 ) == 0 ? "\n" : "" );
                ebx( target, buf, strlen( buf ) );
              }
              //cl = cl->next;
            }
...
,  };
  ...
  for ( ; n <= 9; n++ )
  {
    if ( table[ n ].domain[0] != 0 )
    {
      cookie_domain_flag_set( table[ n ].domain[0], table[ n ].flag );
      if ( table[ n ].once != 0 && table[ n ].domain[0] != 0 )
      {
