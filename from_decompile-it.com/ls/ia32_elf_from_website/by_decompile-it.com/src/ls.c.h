...
static inline int getfilecon( char *s, char **con )
{
  *(int*)(__errno_location( )) = 95;
  return -1;
}
static inline int lgetfilecon( char *s, char **con )
{
  *(int*)(__errno_location( )) = 95;
  return -1;
}
static inline _Bool dot_or_dotdot( char *file_name )
{
  if ( file_name[0] == '.' )
  {
    char sep = file_name[ file_name[1] == '.' ? 2 : 1 ];
  }
  else
  {
  }
...
static inline struct timespec get_stat_ctime( struct stat *st )
{
  *ebp_8 = st->st_ctim.tv_sec;
  *(int*)(ebp_8 + 4) = st->st_ctim.tv_nsec;
  return ebp_8;
}
static inline int timespec_cmp( struct timespec a, struct timespec b )
{
  return a.tv_sec < b.tv_sec ? -1 : a.tv_sec <= b.tv_sec ? a.tv_nsec - b.tv_nsec : 1;
}
static inline struct timespec get_stat_mtime( struct stat *st )
{
  *ebp_8 = st->st_mtim.tv_sec;
  *(int*)(ebp_8 + 4) = st->st_mtim.tv_nsec;
  return ebp_8;
}
static inline struct timespec get_stat_atime( struct stat *st )
{
  *ebp_8 = st->st_atim.tv_sec;
  *(int*)(ebp_8 + 4) = st->st_atim.tv_nsec;
  return ebp_8;
}
...
/******                                                                             ******/
/*****************************************************************************************/
static char filetype_letter[11];
...
static char *time_style_args[5];
static enum time_style  time_style_types[4];
static enum time_type  time_type;
static enum sort_type  sort_type;
static _Bool sort_reverse;
static _Bool print_owner;
static _Bool print_author;
static _Bool print_group;
static _Bool numeric_ids;
static _Bool print_block_size;
static int human_output_opts;
static uintmax_t output_block_size;
static uintmax_t file_output_block_size;
static _Bool dired;
static enum indicator_style  indicator_style;
static char *indicator_style_args[5];
static enum indicator_style  indicator_style_types[4];
static _Bool print_with_color;
static char *indicator_name[21];
static struct bin_str  color_indicator[20];
...
static char *long_time_format[2];
static sigset_t caught_signals;
static sig_atomic_t interrupt_signal;
static sig_atomic_t stop_signal_count;
static int exit_status;
static struct option  long_options[42];
static char *format_args[8];
static enum format  format_types[7];
static char *sort_args[6];
static enum sort_type  sort_types[5];
static char *time_args[6];
static enum time_type  time_types[5];
static char *color_args[10];
static enum color_type  color_types[9];
...
static qsortFunc sort_functions[7][2][2][2];
extern char *program_name;
extern int ls_mode;
extern argmatch_exit_fn argmatch_die;
extern int exit_failure;
extern char *quoting_style_args[8];
extern enum quoting_style  quoting_style_vals[7];
/*****************************************************************************************/
/******                                                                             ******/
/******                      Inline Function Definitions                            ******/
/******                                                                             ******/
