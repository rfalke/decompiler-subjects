...
static char *memcpy_lowcase( char *dest, char *src, size_t len );
static char *memcpy_uppcase( char *dest, char *src, size_t len );
static size_t strftime_case_( _Bool upcase, char *s, size_t maxsize, char *format, struct tm *tp, int ut, int ns );
...
size_t strftime_case_( _Bool upcase, char *s, size_t maxsize, char *format, struct tm *tp, int ut, int ns )
{
  int eax;
    ...
    case 37:
      ...
      switch ( f[0] )
      {
        if ( f[0] - 48 <= 9 )
        {
          ...
          do
          {
            if ( width > 0xccccccc || ( width == 214748364 && f[0] - 48 > 7 ) )
              width = 0x7fffffff;
            else
            {
              width *= 10;
            ...
            if ( len != 0 )
            {
              ...
              else
              {
                if ( p != 0 )
                {
                  ...
                  if ( to_lowcase != 0 )
                    memcpy_lowcase( p, &ubuf[1], _n );
                  else
                  if ( to_uppcase == 0 )
                    memcpy( p, &ubuf[1], _n );
                    p = &p[ _n ];
                  memcpy_uppcase( p, &ubuf[1], _n );
                  p = &p[ _n ];
                }
                i += _incr;
              }
        ...
        case 67:
          if ( modifier != 79 )
          {
            ...
            {
              int century = ( ( (/*HI*/int)( 0x51eb851f * tp->tm_year ) >> 5 ) - ( tp->tm_year >> 31 ) ) + 19;
              century -= tp->tm_year - ( ( ( (/*HI*/int)( 0x51eb851f * tp->tm_year ) >> 5 ) - ( tp->tm_year >> 31 ) ) * 100 ) >= 0 || century < 1 ? 0 : 1;
              digits = 2;
                ...
                }
              {
                ...
                else
                {
                  if ( p != 0 )
                  {
                    ...
                    if ( to_lowcase != 0 )
                      memcpy_lowcase( p, bufp, _n );
                    else
                    if ( to_uppcase == 0 )
                      memcpy( p, bufp, _n );
                      p = &p[ _n ];
                    memcpy_uppcase( p, bufp, _n );
                    p = &p[ _n ];
                  }
                  i += _incr;
                }
        ...
        case 68:
          if ( modifier == 0 )
            subfmt = "%m/%d/%y";
          {
            size_t len = strftime_case_( to_uppcase, 0, -1, subfmt, tp, ut, ns );
          {
        ...
        case 78:
          if ( modifier != 69 )
          {
            ...
            else
            {
              ...
              for ( ; j <= 8; j++ )
              {
                number_value = ( (/*HI*/int)( 0x66666667 * number_value ) >> 2 ) - ( number_value >> 31 );
                //j++;
              }
            }
            digits = width;
        ...
        case 103:
          if ( modifier != 69 )
          {
            ...
            {
              year_adjust = -1;
              days = iso_week_days( tp->tm_yday + ( ( ( year - 1 ) & 3 ) == 0 && ( year - 1 - ( ( ( (/*HI*/int)( 0x51eb851f * ( year - 1 ) ) >> 5 ) - ( ( year - 1 ) >> 31 ) ) * 100 ) != 0 || year - 1 - ( ( ( (/*HI*/int)( 0x51eb851f * ( year - 1 ) ) >> 7 ) - ( ( year - 1 ) >> 31 ) ) * 400 ) == 0 ) ? 1 : 0 ) + 365, tp->tm_wday );
            }
            else
            {
              int d = iso_week_days( tp->tm_yday + ( ( ( ( year & 3 ) == 0 && ( year - ( ( ( (/*HI*/int)( 0x51eb851f * year ) >> 5 ) - ( year >> 31 ) ) * 100 ) != 0 || year - ( ( ( (/*HI*/int)( 0x51eb851f * year ) >> 7 ) - ( year >> 31 ) ) * 400 ) == 0 ) ? 1 : 0 ) * -1 ) - 365 ), tp->tm_wday );
              if ( d >= 0 )
              {
                year_adjust = 1;
                days = d;
            ...
            case 103:
            {
              int yy = ( (/*HI*/int)( 0x51eb851f * ( year_adjust + ( tp->tm_year - ( ( ( (/*HI*/int)( 0x51eb851f * tp->tm_year ) >> 5 ) - ( tp->tm_year >> 31 ) ) * 100 ) ) ) ) >> 5 ) - ( ( year_adjust + ( tp->tm_year - ( ( ( (/*HI*/int)( 0x51eb851f * tp->tm_year ) >> 5 ) - ( tp->tm_year >> 31 ) ) * 100 ) ) ) >> 31 );
              yy = ( year_adjust + ( tp->tm_year - ( ( ( (/*HI*/int)( 0x51eb851f * tp->tm_year ) >> 5 ) - ( tp->tm_year >> 31 ) ) * 100 ) ) ) - ( yy * 100 );
              digits = 2;
        ...
        case 121:
          ...
          {
            int yy = ( (/*HI*/int)( 0x51eb851f * tp->tm_year ) >> 5 ) - ( tp->tm_year >> 31 );
            ...
            zone = "";
        {
          ...
          else
          {
            if ( p != 0 )
            {
              ...
              if ( to_lowcase != 0 )
                memcpy_lowcase( p, zone, _n );
              else
              if ( to_uppcase == 0 )
                memcpy( p, zone, _n );
                p = &p[ _n ];
              memcpy_uppcase( p, zone, _n );
              p = &p[ _n ];
            }
            i += _incr;
          }
