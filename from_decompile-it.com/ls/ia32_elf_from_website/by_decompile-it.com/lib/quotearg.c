...
static char *gettext_quote( char *msgid, enum quoting_style  s );
static size_t quotearg_buffer_restyled( char *buffer, size_t buffersize, char *arg, size_t argsize, enum quoting_style  quoting_style, struct quoting_options *o );
static char *quotearg_n_options( int n, char *arg, size_t argsize, struct quoting_options *options );
...
struct quoting_options *clone_quoting_options( struct quoting_options *o )
{
  ...
  struct quoting_options *p = xmemdup( o == 0 ? &default_quoting_options.style : (void*)o, 36 );
  *(int*)(__errno_location( )) = e;
  return p;
}
enum quoting_style  get_quoting_style( struct quoting_options *o )
{
  return *(int*)(o == 0 ? default_quoting_options : o);
}
...
{
  unsigned char uc = c;
  unsigned int *p = ( o == 0 ? default_quoting_options.style : o ) + 4 + ( ( uc >> 5 ) << 2 );
  int shift = uc & 31;
...
size_t quotearg_buffer_restyled( char *buffer, size_t buffersize, char *arg, size_t argsize, enum quoting_style  quoting_style, struct quoting_options *o )
{
  ...
  case c_quoting_style:
    ...
    quote_string_len = 1;
    i = 0;
    for ( ; ( argsize == -1 ? ( arg[ i ] & 255 ) != 0 : i != argsize ) != 0; i++ )
    {
      unsigned char c;
      unsigned char esc;
      if ( backslash_escapes != 0 && quote_string_len != 0 && quote_string_len + i <= argsize && memcmp( &arg[ i ], quote_string, quote_string_len ) == 0 )
      {
        if ( len < buffersize )
          buffer[ len ] = '\\';
        len++;
      ...
      case 0:
        if ( backslash_escapes != 0 )
        {
          ...
          len++;
          c = 48;
          if ( ( ( backslash_escapes ^ 1 ) & 255 ) == 0 && ( o->quote_these_too[ c >> 5 ] & ( 1 << ( c & 31 ) ) ) != 0 )
          {
            if ( len < buffersize )
              buffer[ len ] = '\\';
            len++;
      ...
      case 63:
        ...
        {
          if ( quoting_style == c_quoting_style && i + 2 < argsize && arg[ i + 1 ] == '?' && arg[ i + 2 ] - 33 <= 29 && ( ( 1 << ( arg[ i + 2 ] - 33 ) ) & 0x380051c1 ) != 0 )
          {
            c = arg[ i + 2 ];
            i += 2;
            if ( len < buffersize )
      ...
      case 123:
      case 125:
        if ( ( argsize == -1 ? ( arg[1] & 255 ) != 0 : argsize != 1 ) == 0 )
      case 35:
      case 126:
        if ( i == 0 )
      case 32:
      ...
      case 64:
      {
        ...
        {
          m = 1;
          printable = ( *(short*)(*(int*)(__ctype_b_loc( c )) + ( c * 2 )) & 16384 ) != 0;
        }
        else
        {
          mbstate_t mbstate;
        ...
        if ( m > 1 || ( ( ( backslash_escapes ^ 1 ) & 255 ) == 0 && ( ( printable ^ 1 ) & 255 ) != 0 ) )
        {
          size_t ilim = m + i;
    ...
    if ( i != 0 || quoting_style != shell_quoting_style )
    {
      if ( quote_string != 0 )
      {
        for ( ; ( quote_string[0] & 255 ) != 0; quote_string++ )
        {
          if ( len < buffersize )
...
{
  struct quoting_options *p = o == 0 ? &default_quoting_options : o;
  int e = *(int*)(__errno_location( p ));
...
{
  int e = *(int*)(__errno_location( ebx ));
  unsigned int n0 = n;
  struct slotvec *sv = slotvec;
  if ( n < 0 )
  {
  ...
  if ( nslots <= n0 )
  {
    ...
    if ( n1 > 0x1fffffff )
      xalloc_die( );
    slotvec = sv = xrealloc( preallocated == 0 ? (void*)sv : 0, n1 << 3 );
    if ( preallocated != 0 )
    {
