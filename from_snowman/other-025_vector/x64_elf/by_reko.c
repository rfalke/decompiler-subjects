// subject_bss.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

byte g_b600E20 = 0x00; // 0000000000600E20
uint64 dtor_idx.5888 = 0x00; // 0000000000600E28
// subject_ctors.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_ptr600BD0 = &g_tFFFFFFFF; // 0000000000600BD0
// subject_dtors.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_a600BE0[] = // 0000000000600BE0
	{
	};
// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400A78: void _fini()
void _fini()
{
	__do_global_dtors_aux();
}

// subject_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * __gmon_start___GOT = null; // 0000000000600DC8
// subject_got_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * _ZSt20__throw_length_errorPKc_GOT = &g_t400546; // 0000000000600DE8
<anonymous> * _ZdlPv_GOT = &g_t400556; // 0000000000600DF0
<anonymous> * __libc_start_main_GOT = &g_t400566; // 0000000000600DF8
<anonymous> * memmove_GOT = &g_t400576; // 0000000000600E00
<anonymous> * _Znwm_GOT = &g_t400586; // 0000000000600E08
// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400510: void _init()
// Called from:
//      __libc_csu_init
void _init()
{
	call_gmon_start();
	frame_dummy();
	__do_global_ctors_aux();
}

// subject_jcr.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw600BF0 = 0x00; // 0000000000600BF0
// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> g_t400546 = <code>; // 0000000000400546
<anonymous> g_t400556 = <code>; // 0000000000400556
<anonymous> g_t400566 = <code>; // 0000000000400566
<anonymous> g_t400576 = <code>; // 0000000000400576
<anonymous> g_t400586 = <code>; // 0000000000400586
// subject_rodata.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

const char g_str400A8C[] = "vector::_M_range_insert"; // 0000000000400A8C
// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400590: void main()
void main()
{
}

// 0000000000400594: void _start(Register (ptr64 Eq_11) rdx, Stack Eq_12 qwArg00)
void _start(void (* rdx)(), Eq_12 qwArg00)
{
	__align((const char *) fp + 8);
	__libc_start_main(&g_t400590, qwArg00, (const char *) fp + 8, &g_t4009B0, &g_t4009A0, rdx, fp);
	__hlt();
}

// 00000000004005C0: void call_gmon_start()
// Called from:
//      _init
void call_gmon_start()
{
	if (__gmon_start__ != 0x00)
		__gmon_start__();
}

// 00000000004005E0: void __do_global_dtors_aux()
// Called from:
//      _fini
void __do_global_dtors_aux()
{
	if (g_b600E20 == 0x00)
	{
		uint64 rax_16 = dtor_idx.5888;
		while (rax_16 < 0x00)
		{
			uint64 rax_23 = rax_16 + 0x01;
			dtor_idx.5888 = rax_23;
			(*((const char *) g_a600BE0 + rax_23 * 0x08))();
			rax_16 = dtor_idx.5888;
		}
		g_b600E20 = 0x01;
	}
}

// 0000000000400650: void frame_dummy()
// Called from:
//      _init
void frame_dummy()
{
}

// 0000000000400680: void _Z1fRSt6vectorIiSaIiEES2_(Register (ptr64 Eq_71) rsi, Register (ptr64 Eq_72) rdi, Register Eq_73 xmm0)
void _Z1fRSt6vectorIiSaIiEES2_(struct Eq_71 * rsi, struct Eq_72 * rdi, Eq_73 xmm0)
{
	_ZNSt6vectorIiSaIiEE15_M_range_insertIN9__gnu_cxx17__normal_iteratorIPiS1_EEEEvS6_T_S7_St20forward_iterator_tag(rsi->ptr0008, rsi->ptr0000, rdi->ptr0008, rdi, xmm0);
}

// 00000000004006A0: void _ZNSt6vectorIiSaIiEE15_M_range_insertIN9__gnu_cxx17__normal_iteratorIPiS1_EEEEvS6_T_S7_St20forward_iterator_tag(Register (ptr64 void) rcx, Register (ptr64 void) rdx, Register (ptr64 void) rsi, Register (ptr64 Eq_72) rdi, Register Eq_73 xmm0)
// Called from:
//      _Z1fRSt6vectorIiSaIiEES2_
void _ZNSt6vectorIiSaIiEE15_M_range_insertIN9__gnu_cxx17__normal_iteratorIPiS1_EEEEvS6_T_S7_St20forward_iterator_tag(void * rcx, void * rdx, void * rsi, struct Eq_72 * rdi, Eq_73 xmm0)
{
	if (rdx == rcx)
		return;
	int64 r13_29 = rcx - rdx;
	void * r14_26 = rdi->ptr0008;
	Eq_98 r13_260 = r13_29 >> 0x02;
	if (r13_29 >> 0x02 <= rdi->t0010 - r14_26 >> 0x02)
	{
		Eq_98 rdx_130;
		int64 r15_36 = r14_26 - rsi;
		if (r13_29 >> 0x02 < r15_36 >> 0x02)
		{
			Eq_98 r15_92 = (r13_29 >> 0x02) * 0x04;
			void * rax_107 = r14_26;
			void * rcx_96 = r14_26 - r15_92;
			if (r15_92 >> 0x02 != 0x00)
			{
				memmove(r14_26, rcx_96, __align(r15_92, 4));
				rax_107 = rdi->ptr0008;
			}
			Mem117[rdi + 0x08:word64] = rax_107 + r15_92;
			int64 rcx_111 = rcx_96 - rsi;
			if (rcx_111 >> 0x02 == 0x00)
			{
				rdx_130 = r15_92;
				if (r13_29 >> 0x02 == 0x00)
					return;
			}
			else
			{
				Eq_98 rdx_121 = (rcx_111 >> 0x02) * 0x04;
				memmove(r14_26 - rdx_121, rsi, rdx_121);
				rdx_130 = r15_92;
				if (r13_29 >> 0x02 == 0x00)
					return;
			}
		}
		else
		{
			Eq_98 r8_40 = (r15_36 >> 0x02) * 0x04;
			word64 r9_41 = rdx + r8_40;
			int64 rcx_42 = rcx - r9_41;
			if (rcx_42 >> 0x02 != 0x00)
			{
				memmove(r14_26, r9_41, (rcx_42 >> 0x02) * 0x04);
				r14_26 = rdi->ptr0008;
			}
			void * rdi_62 = (const char *) r14_26 + ((r13_29 >> 0x02) - (r15_36 >> 0x02)) * 0x04;
			rdi->ptr0008 = rdi_62;
			if (r15_36 >> 0x02 != 0x00)
			{
				memmove(rdi_62, rsi, r8_40);
				rdi_62 = rdi->ptr0008;
			}
			Mem87[rdi + 0x08:word64] = r8_40 + rdi_62;
			int64 r9_81 = r9_41 - rdx;
			if (r9_81 >> 0x02 == 0x00)
				return;
			rdx_130 = (r9_81 >> 0x02) * 0x04;
		}
		memmove(rsi, rdx, rdx_130);
		return;
	}
	Eq_116 r15_168;
	void * r14_187;
	void * rsi_156 = rdi->ptr0000;
	int64 r14_159 = r14_26 - rsi_156;
	Eq_124 r14_160 = r14_159 >> 0x02;
	if (r13_29 >> 0x02 <= 0x3FFFFFFFFFFFFFFF - (r14_159 >> 0x02))
	{
		int64 rax_167 = r13_29 >> 0x02;
		r15_168.u0 = ~0x03;
		if (r14_159 >> 0x02 >= r13_29 >> 0x02)
			rax_167 = r14_159 >> 0x02;
		r14_160 = (r14_159 >> 0x02) + rax_167;
		if (r14_160 < 0x00 || r14_160 > 0x3FFFFFFFFFFFFFFF)
		{
l0000000000400738:
			void * rax_225;
			word64 r8_226;
			word64 r9_227;
			_Znwm();
			rsi_156 = rdi->ptr0000;
			r14_187 = rax_225;
			goto l0000000000400746;
		}
		if (r14_160 == 0x00)
		{
			r15_168.u1 = 0x00;
			r14_187 = null;
l0000000000400746:
			Eq_98 rcx_237 = 0x00;
			int64 rax_239 = rsi - rsi_156;
			if (rax_239 >> 0x02 != 0x00)
			{
				Eq_98 rcx_244 = (rax_239 >> 0x02) * 0x04;
				memmove(r14_187, rsi_156, rcx_244);
				rcx_237 = rcx_244;
			}
			word64 rcx_254 = rcx_237 + r14_187;
			if (r13_29 >> 0x02 != 0x00)
			{
				r13_260 = __align(r13_29, 4);
				memmove(rcx_254, rdx, r13_260);
				rsi_156 = rdx;
			}
			word64 r13_280 = r13_260 + rcx_254;
			Eq_98 rbp_282 = 0x00;
			int64 rax_285 = rdi->ptr0008 - rsi;
			if (rax_285 >> 0x02 != 0x00)
			{
				rbp_282 = (rax_285 >> 0x02) * 0x04;
				memmove(r13_280, rsi, rbp_282);
				rsi_156 = rsi;
			}
			void * rdi_297 = rdi->ptr0000;
			word64 rbp_300 = rbp_282 + r13_280;
			if (rdi_297 != null)
				(xmm0, rdi_297, rsi_156);
			rdi->ptr0000 = r14_187;
			rdi->ptr0008 = rbp_300;
			Mem329[rdi + 0x10:word64] = r14_187 + r15_168;
			return;
		}
	}
	else
		std::__throw_length_error("vector::_M_range_insert");
	r15_168 = r14_160 * 0x04;
	goto l0000000000400738;
}

// 00000000004009A0: void __libc_csu_fini()
void __libc_csu_fini()
{
}

// 00000000004009B0: void __libc_csu_init(Register word64 rdx, Register word64 rsi, Register word32 edi)
void __libc_csu_init(word64 rdx, word64 rsi, word32 edi)
{
	_init();
	int64 rbp_19 = 0x00600BCC - 0x00600BCC;
	if (rbp_19 >> 0x03 != 0x00)
	{
		Eq_349 rbx_39 = 0x00;
		do
		{
			((<anonymous> *[]) 0x00600BCC)[rbx_39]();
			rbx_39 = (word64) rbx_39.u1 + 1;
		} while (rbx_39 < rbp_19 >> 0x03);
	}
}

// 0000000000400A40: void __do_global_ctors_aux()
// Called from:
//      _init
void __do_global_ctors_aux()
{
	<anonymous> * rax_12 = g_ptr600BD0;
	if (rax_12 != (<anonymous> *) ~0x00)
	{
		word64 * rbx_16 = &g_ptr600BD0;
		do
		{
			rax_12();
			rbx_16 -= 0x08;
			rax_12 = *rbx_16;
		} while (rax_12 != (<anonymous> *) ~0x00);
	}
}

