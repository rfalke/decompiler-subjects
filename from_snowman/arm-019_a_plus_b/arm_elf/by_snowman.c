...
void deregister_tm_clones() {
    ...
    r3_1 = d;
    if (reinterpret_cast<uint32_t>(r3_1 - 0x105a4) <= 6) {
        return;
    } else {
        if (1) {
            return;
...
void f() {
    __asm__("vstr d0, [fp, #-0xc]");
    __asm__("vstr d1, [fp, #-0x14]");
    __asm__("vldr d17, [fp, #-0xc]");
    __asm__("vldr d16, [fp, #-0x14]");
    __asm__("vadd.f64 d16, d17, d16");
    __asm__("vmov.f64 d0, d16");
    return;
}
...
void _start(int32_t r0) {
    ...
    r0_4 = abort();
    if (z5) {
        *reinterpret_cast<uint32_t*>(&v6) = reinterpret_cast<uint32_t>(r0_4) & (reinterpret_cast<uint32_t>(r0_4) >> r4_7 | reinterpret_cast<uint32_t>(r0_4) << reinterpret_cast<uint32_t>(32 - r4_8));
    }
    if (z9) {
        *r0_4 = v6;
    }
