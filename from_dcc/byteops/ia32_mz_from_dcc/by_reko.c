// subject_0800.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

struct seg0800_t * g_ptr8000 = &g_t07BA; // 00008000
// 0800:01FA: void _main(Register byte ch, Register Eq_6 ds)
void _main(byte ch, Eq_6 ds)
{
	_printf(SEQ(ch, 0xC0), 0, 0xAE, ds, 404);
}

// 0800:0796: void __LONGTOA(Register Eq_6 ds, Stack Eq_21 bArg02, Stack Eq_22 bArg04, Stack Eq_23 wArg06, Stack (memptr Eq_6 byte) wArg08, Stack Eq_25 dwArg0A)
// Called from:
//      __VPRINTER
void __LONGTOA(Eq_6 ds, Eq_21 bArg02, Eq_22 bArg04, Eq_23 wArg06, byte Eq_6::* wArg08, Eq_25 dwArg0A)
{
	Eq_26 fp;
	uint16 wArg0A = (word16) dwArg0A;
	Eq_29 wArg0C = SLICE(dwArg0A, word16, 16);
	byte Eq_6::* di_128 = wArg08;
	cu8 bl_28 = (byte) wArg06;
	if (wArg06 > 0x24 || bl_28 < 0x02)
	{
l0800_0804:
		ds->*di_128 = 0x00;
		return;
	}
	uint16 ax_31 = wArg0A;
	Eq_29 cx_33 = wArg0C;
	if (wArg0C < 0x00 && bArg04 != 0x00)
	{
		ds->*wArg08 = 0x2D;
		ui32 cx_ax_175 = -dwArg0A;
		di_128 = wArg08 + 1;
		ax_31 = (word16) cx_ax_175;
		cx_33 = SLICE(cx_ax_175, word16, 16);
	}
	byte Eq_6::* si_100 = fp - 36;
	if (cx_33 != 0x00)
	{
		do
		{
			uint32 dx_ax_57 = (uint32) cx_33;
			uint32 dx_ax_65 = SEQ((uint16) (dx_ax_57 % wArg06), ax_31);
			ds->*si_100 = (byte) (dx_ax_65 % wArg06);
			Eq_29 ax_61 = (uint16) (dx_ax_57 /u wArg06);
			cx_33 = ax_61;
			ax_31 = (uint16) (dx_ax_65 /u wArg06);
			++si_100;
		} while (ax_61 != 0x00);
l0800_07E7:
		if (ax_31 == 0x00)
		{
			int16 cx_107 = si_100 + (36 - fp);
			do
			{
				byte al_120;
				--si_100;
				cu8 al_114 = ds->*si_100;
				if (al_114 < 0x0A)
					al_120 = al_114 + 0x30;
				else
					al_120 = (byte) bArg02.u1 + (al_114 - 0x0A);
				ds->*di_128 = al_120;
				++di_128;
				--cx_107;
			} while (cx_107 != 0x00);
			goto l0800_0804;
		}
	}
	uint32 dx_ax_85 = (uint32) ax_31;
	ds->*si_100 = (byte) (dx_ax_85 % wArg06);
	ax_31 = (uint16) (dx_ax_85 /u wArg06);
	++si_100;
	goto l0800_07E7;
}

// 0800:0920: void _printf(Register word16 cx, Register int16 dx, Register uint16 bx, Register Eq_6 ds, Stack word16 wArg02)
// Called from:
//      _main
void _printf(word16 cx, int16 dx, uint16 bx, Eq_6 ds, word16 wArg02)
{
	mp16 fp;
	__VPRINTER(cx, dx, bx, ds, fp + 4, wArg02);
}

// 0800:0B19: void __REALCVT(Register Eq_6 ds)
// Called from:
//      __VPRINTER
void __REALCVT(Eq_6 ds)
{
	Mem0[ds:0x0426:word16]();
}

// 0800:0B1D: Register word16 fn0800_0B1D(Sequence (ptr32 Eq_145) es_di, Register Eq_6 ds, Stack Eq_6 wArg02)
// Called from:
//      __VPRINTER
word16 fn0800_0B1D(struct Eq_145 * es_di, Eq_6 ds, Eq_6 wArg02)
{
	word16 di = (word16) es_di;
	bcu8 dh_13 = SLICE(wArg02, byte, 8);
	Mem21[es_di + 0x00:byte] = Mem7[ds:CONVERT(dh_13 >>u 0x04, byte, uint16) + 0x03AF:byte];
	Mem27[es_di + 1:byte] = Mem21[ds:CONVERT(dh_13 & 0x0F, byte, uint16) + 0x03AF:byte];
	bcu8 dl_29 = (byte) wArg02;
	Mem33[es_di + 2:byte] = Mem27[ds:CONVERT(dl_29 >>u 0x04, byte, uint16) + 0x03AF:byte];
	Mem41[es_di + 3:byte] = Mem33[ds:CONVERT(dl_29 & 0x0F, byte, uint16) + 0x03AF:byte];
	return di + 4;
}

// 0800:0B48: void __VPRINTER(Register word16 cx, Register int16 dx, Register uint16 bx, Register Eq_6 ds, Stack mp16 wArg02, Stack word16 wArg04)
// Called from:
//      _printf
void __VPRINTER(word16 cx, int16 dx, uint16 bx, Eq_6 ds, mp16 wArg02, word16 wArg04)
{
	bLoc57_1259 = 0x50;
	sp_20 = fp - 0xA0;
	wLoc98_1262 = fp - 86;
l0800_0BAB:
	di_29 = wLoc98_1263;
l0800_0BAF:
	si_33 = wArg04_1264;
	while (true)
	{
		al_37 = ds->*si_35;
		si_38 = si_35 + 1;
		al_39 = al_37;
		if (al_37 == 0x00)
			break;
		if (al_37 == 0x25)
		{
			al_49 = ds->*si_38;
			si_50 = si_38 + 1;
			if (al_49 != 0x25)
			{
				wLoc98_1266 = di_53;
				ch_1171 = 0x00;
				wLoc8E_1267 = 0x00;
				wLoc9A_1268 = 0x00;
				bLoc8F_1269 = 0x00;
				wLoc94_1270 = ~0x00;
				wLoc92_1271 = ~0x00;
				while (true)
				{
					ax_67 = (uint16) al_66;
					bl_70 = (byte) ax_67;
					bh_75 = SLICE(ax_67, byte, 8);
					dx_68 = ax_67;
					dl_137 = (byte) ax_67;
					bx_76 = SEQ(bh_75, bl_70 - 0x20);
					cx_1191 = SEQ(ch_868, 0x00);
					if (bl_70 >= 0x80)
						break;
					bl_78 = (ds->*bx_76).b03BF;
					bx_79 = SEQ(bh_75, bl_78);
					cx_1188 = SEQ(ch_868, 0x00);
					if (bx_79 > 0x17)
						goto l0800_1035;
					bx_85 = bx_79 << 0x01;
					cx_1189 = SEQ(ch_868, 0x00);
					switch (bx_79)
					{
					case 0x00:
						cx_1194 = SEQ(ch_868, 0x00);
						if (ch_868 > 0x00)
							goto l0800_0C4A;
						if (bLoc8F_1360 != 0x2B)
							bLoc8F_1482 = dl_137;
						break;
					case 0x01:
						cx_1193 = SEQ(ch_868, 0x00);
						if (ch_868 > 0x00)
							goto l0800_0C4A;
						wLoc9A_1481 = wLoc9A_1272 | 0x01;
						break;
					case 0x02:
						ax_967.u2 = (ds->*wArg02_1276).t0000.u2;
						wArg02_1478 = (union Eq_6 Eq_6::*) &wArg02_1276->t0002.u0;
						if (ch_868 < 0x02)
						{
							wLoc94_1480 = ax_967;
							ch_978 = 0x03;
							goto l0800_0CBA;
						}
						cx_1198 = SEQ(ch_868, 0x00);
						if (ch_868 != 0x04)
							goto l0800_0CA3;
						wLoc92_1479 = ax_967;
						ch_975 = ch_868 + 0x01;
						break;
					case 0x03:
						cx_1192 = SEQ(ch_868, 0x00);
						if (ch_868 > 0x00)
							goto l0800_0C4A;
						wLoc9A_1477 = wLoc9A_1272 | 0x02;
						break;
					case 0x04:
						cx_1196 = SEQ(ch_868, 0x00);
						if (ch_868 >= 0x04)
							goto l0800_0CA3;
						ch_958 = 0x04;
						break;
					case 0x05:
						goto l0800_0CD5;
					case 0x06:
						wLoc9A_1470 = wLoc9A_1272 | 0x10;
						ch_904 = 0x05;
						break;
					case 0x07:
						wLoc9A_1469 = (wLoc9A_1272 | 0x0100) & ~0x10;
						ch_898 = 0x05;
						break;
					case 0x08:
						wLoc9A_1467 = wLoc9A_1272 & ~0x10 | 0x80;
						ch_889 = 0x05;
						break;
					case 0x09:
						if (ch_868 <= 0x00)
						{
							if ((wLoc9A_1272 & 0x02) != 0x00)
								goto l0800_0CBA;
							wLoc9A_1465 = wLoc9A_1272 | 0x08;
							ch_880 = 0x01;
						}
						else
						{
l0800_0CD5:
							dx_908 = bx_79;
							ax_912 = (int16) ((byte) ax_67 - 0x30);
							if (ch_868 <= 0x02)
							{
								ch_938 = 0x02;
								wLoc94_1475 = ax_912;
								if (wLoc94_1347 < 0x00)
									goto l0800_0CBA;
								wLoc94_1476 = ax_912 + wLoc94_1347 * 0x0A;
							}
							else
							{
								cx_1197 = SEQ(ch_868, 0x00);
								if (ch_868 != 0x04)
								{
l0800_0CA3:
									goto l0800_1035;
								}
								wLoc92_1472 = ax_912;
								if (wLoc92_1289 >= 0x00)
									wLoc92_1473 = ax_912 + wLoc92_1289 * 0x0A;
								else
								{
l0800_0CBA:
								}
							}
						}
						break;
					case 0x0A:
						dx_ax_378 = (int32) (ds->*wArg02_1276).t0000.u2;
						bLoc95_1317 = 0x01;
						ax_404 = (word16) dx_ax_378;
						dx_412 = SLICE(dx_ax_378, word16, 16);
						bx_1967 = SEQ(0x0A, bl_78 << 0x01);
						goto l0800_0D77;
					case 11:
						bx_1979 = SEQ(0x08, bl_78 << 0x01);
						goto l0800_0D4F;
					case 0x0C:
						bx_1973 = SEQ(0x0A, bl_78 << 0x01);
						goto l0800_0D54;
					case 0x0D:
						bx_1980 = SEQ(0x10, dl_137 + 233);
l0800_0D4F:
						bLoc8F_1312 = 0x00;
						bx_1972 = bx_1981;
l0800_0D54:
						bLoc95_1313 = 0x00;
						ax_368.u2 = (ds->*wArg02_1276).t0000.u2;
						dx_369 = 0x00;
						bx_1966 = bx_1974;
l0800_0D77:
						di_380 = (struct Eq_608 Eq_6::*) ((char *) &wArg02_1276->t0000 + 1);
						bh_449 = SLICE(bx_1968, byte, 8);
						di_381 = (union Eq_6 Eq_6::*) &di_380->w0001;
						wArg04_1319 = si_101;
						if ((wLoc9A_1272 & 0x10) != 0x00)
						{
							dx_393 = (ds->*di_380).w0001;
							di_395 = (struct Eq_668 Eq_6::*) (&di_380->w0001 + 1);
						}
						wArg02_1323 = di_397;
						if (ax_403 == 0x00)
						{
							dx_413 = dx_410;
							dh_1170 = SLICE(dx_410, byte, 8);
							if (dx_410 != 0x00)
								goto l0800_0DC6;
							bx_1169 = bx_1968;
							if (wLoc92_1289 == 0x00)
							{
								di_839 = di_53;
								cx_840 = wLoc94_1347;
								if (wLoc94_1347 != 0x00 && wLoc94_1347 != ~0x00)
								{
									if ((wLoc9A_1272 & 0x08) != 0x00)
										dl_850 = 0x30;
									else
										dl_849 = 0x20;
									do
									{
										dx_862 = SEQ(dh_1170, dl_851);
										di_863 = fn0800_0B6E(SEQ(ds, di_854), SEQ(ss, fp - 2), dl_851, cx_860, dx_862, bx_1968);
										cx_867 = cx_860 - 0x01;
									} while (cx_867 != 0x00);
								}
								goto l0800_0BAF;
							}
						}
						else
						{
l0800_0DC6:
							wLoc9A_1326 = wLoc9A_1272 | 0x04;
						}
						sp_434 = sp_1222 - 2;
						(ss->*sp_434).t0000.u2 = (struct Eq_1338 *) dx_410;
						(ss->*sp_434).tFFFFFFFE.u0 = (word16) ax_403;
						(ss->*sp_434).ptrFFFFFFFC = fp - 0x89;
						ax_454.u0 = (int16) bh_449;
						(ss->*sp_434).tFFFFFFFA.u0 = (int16) ax_454;
						(ss->*sp_434).tFFFFFFF8.u1 = (word16) SEQ(SLICE(ax_454, byte, 8), bLoc95_1333);
						(ss->*sp_434).tFFFFFFF6.u0 = (ci16) bx_1968;
						__LONGTOA(ds, (ss->*sp_434).tFFFFFFF6.u1, (ss->*sp_434).tFFFFFFF8.u0, (ss->*sp_434).tFFFFFFFA.u0, (ss->*sp_434).ptrFFFFFFFC, (ss->*sp_434).tFFFFFFFE.u1);
						(ss->*sp_434).t0000.u2 = (struct Eq_1338 *) ss;
						es_485.u2 = (ss->*sp_434).t0000.u2;
						sp_486 = (struct Eq_759 Eq_6::*) ((char *) &sp_434->t0000 + 2);
						dx_488 = wLoc92_1289;
						if (wLoc92_1289 > 0x00)
						{
							es_di_1970 = SEQ(es_485, fp - 0x89);
							goto l0800_0F0C;
						}
						else
						{
							es_di_1975 = SEQ(es_485, fp - 0x89);
							goto l0800_0EFC;
						}
					case 0x0E:
						sp_280 = sp_1222 - 2;
						(ss->*sp_280).t0000.u2 = (ds->*wArg02_1276).t0000.u2;
						di_277 = fp - 0x8A;
						wArg02_1306 = (union Eq_6 Eq_6::*) &wArg02_1276->t0002.u0;
						if ((wLoc9A_1272 & 0x20) != 0x00)
						{
							(ss->*sp_280).tFFFFFFFE.u2 = (ds->*wArg02_1276).t0002.u2;
							(ss->*sp_280).tFFFFFFFC.u2 = (struct Eq_1338 *) ss;
							es_298.u2 = (ss->*sp_280).tFFFFFFFC.u2;
							di_304 = fn0800_0B1D(SEQ(es_298, fp - 0x8A), ds, (ss->*sp_280).tFFFFFFFE.u2);
							es_298->*di_304 = 0x3A;
							wArg02_1307 = (struct Eq_1149 Eq_6::*) ((char *) &wArg02_1276->t0002 + 2);
							di_314 = di_304 + 1;
						}
						(ss->*sp_280).tFFFFFFFE.u2 = (struct Eq_1338 *) ss;
						es_319.u2 = (ss->*sp_280).tFFFFFFFE.u2;
						di_327 = fn0800_0B1D(SEQ(es_319, di_321), ds, (ss->*sp_280).t0000.u2);
						ds->*di_327 = 0x00;
						sp_334 = (struct Eq_1107 Eq_6::*) ((char *) &sp_280->t0000 + 2);
						wLoc9A_1310 = wLoc9A_1272 & ~0x04;
						di_342 = di_327 - (fp - 0x8A);
						dx_346 = wLoc92_1289;
						if (wLoc92_1289 <= di_342)
							dx_349 = di_342;
						es_di_1976 = SEQ(es_319, fp - 0x8A);
						goto l0800_0EFC;
					case 0x0F:
						cx_217 = wLoc92_1289;
						if (wLoc92_1289 < 0x00)
							cx_223 = 0x06;
						sp_225 = sp_1222 - 2;
						(ss->*sp_225).ptr0000 = wArg02_1276;
						(ss->*sp_225).wFFFFFFFE = cx_230;
						(ss->*sp_225).ptrFFFFFFFC = fp - 0x89;
						(ss->*sp_225).wFFFFFFFA = ax_67;
						(ss->*sp_225).wFFFFFFF8 = wLoc9A_1272 & 0x01;
						if ((wLoc9A_1272 & 0x80) != 0x00)
						{
							ax_256 = 0x02;
							wLoc06_1299 = 0x04;
						}
						else if ((wLoc9A_1272 & 0x0100) != 0x00)
						{
							ax_254 = 0x08;
							wLoc06_1298 = 0x0A;
						}
						else
						{
							wLoc06_1297 = 0x08;
							ax_253 = 0x06;
						}
						(ss->*sp_225).wFFFFFFF6 = ax_260;
						__REALCVT(ds);
						(ss->*sp_225).tFFFFFFF4.u2 = (struct Eq_1338 *) ss;
						wArg02_1303 = (struct Eq_1190 Eq_6::*) (wArg02_1276 + wLoc06_1300 /16 6);
						sp_273 = sp_225 - 0x0A;
						es_di_1977 = (ptr32) (fp - 0x89);
l0800_0EFC:
						es_513 = SLICE(es_di_1978, selector, 16);
						di_510 = (word16) es_di_1978;
						if ((wLoc9A_1337 & 0x08) != 0x00)
						{
							dx_504 = wLoc94_1347;
							es_di_1969 = es_di_1978;
							if (wLoc94_1347 > 0x00)
							{
l0800_0F0C:
								es_512 = SLICE(es_di_1971, selector, 16);
								di_509 = (word16) es_di_1971;
								cx_516 = fn0800_0B61(di_509, es_512);
								if (*es_di_1971 == 0x2D)
									cx_524 = cx_516 - 0x01;
								dx_529 = dx_527 - cx_528;
								if (dx_529 > 0x00)
									wLoc8E_1353 = dx_529;
							}
						}
						if (bLoc8F_1354 != 0x00 && es_547->*di_549 != 0x2D)
						{
							di_560 = di_549 - 0x01;
							es_547->*di_560 = bLoc8F_1354;
							v45_553 = wLoc8E_1368 - 0x01;
							wLoc8E_1384 = v45_553 + (word16) (v45_553 < 0x00);
						}
						cx_566 = fn0800_0B61(di_563, es_547);
						goto l0800_0F3D;
					case 0x10:
						sp_200 = sp_1222 - 2;
						(ss->*sp_200).u2 = (struct Eq_1338 *) ss;
						wArg02_1292 = (union Eq_6 Eq_6::*) &wArg02_1276->t0002.u0;
						es_202.u2 = (ss->*sp_200).u2;
						sp_203 = (union Eq_6 Eq_6::*) ((char *) sp_200 + 2);
						di_204 = fp - 0x89;
						cx_212 = 0x01;
						goto l0800_0F3D;
					case 0x11:
						if ((wLoc9A_1272 & 0x20) == 0x00)
						{
							di_154.u2 = (ds->*wArg02_1276).t0000.u2;
							sp_157 = sp_1222 - 2;
							(ss->*sp_157).u2 = (struct Eq_1338 *) ds;
							wArg02_1286 = (union Eq_6 Eq_6::*) &wArg02_1276->t0002.u0;
							es_159.u2 = (ss->*sp_157).u2;
							di_161 = di_154;
							v75_1253 = di_154 != 0x00;
						}
						else
						{
							es_di_144.u1 = (ds->*wArg02_1276).t0000.u1;
							es_147 = SLICE(es_di_144, selector, 16);
							di_149 = (word16) es_di_144;
							wArg02_1285 = (union Eq_6 Eq_6::*) ((char *) &wArg02_1276->t0002 + 2);
							v76_1254 = es_di_144 != 0x00;
						}
						if (!v74_1252)
						{
							sp_169 = sp_1222 - 2;
							(ss->*sp_169).u2 = (struct Eq_1338 *) ds;
							es_173.u2 = (ss->*sp_169).u2;
							di_175 = 0x03A8;
						}
						sp_1235 = sp_1222;
						cx_181 = fn0800_0B61(di_176, es_178);
						if (cx_181 > wLoc92_1289)
							cx_193 = wLoc92_1289;
l0800_0F3D:
						wArg04_1550 = si_101;
						si_573 = di_571;
						di_583 = di_53;
						bx_584 = wLoc94_1347;
						if ((wLoc9A_1413 & 0x05) == 0x05)
						{
							if (dl_137 != 111)
							{
								if (dl_137 == 0x78 || dl_137 == 88)
								{
									v42_605 = wLoc8E_1441 - 0x02;
									wLoc9A_1439 = wLoc9A_1413 | 0x40;
									bx_604 = wLoc94_1347 - 0x02;
									wLoc8E_1446 = v42_605;
									if (v42_605 < 0x00)
										wLoc8E_1447 = 0x00;
								}
l0800_0F8A:
								cx_624 = cx_614 + wLoc8E_1449;
								if ((wLoc9A_1451 & 0x02) == 0x00)
								{
									while (bx_629 > cx_624)
									{
										di_641 = fn0800_0B6E(SEQ(ds, di_635), SEQ(ss, fp - 2), 0x20, cx_624, dx_681, bx_629);
										bx_645 = bx_629 - 0x01;
									}
								}
								if ((wLoc9A_1451 & 0x40) != 0x00)
									di_699 = fn0800_0B6E(SEQ(ds, fn0800_0B6E(SEQ(ds, di_669), SEQ(ss, fp - 2), 0x30, cx_624, dx_681, bx_675)), SEQ(ss, fp - 2), dl_137, cx_624, dx_681, bx_675);
								dx_707 = wLoc8E_1449;
								if (wLoc8E_1449 > 0x00)
								{
									cx_713 = cx_624 - wLoc8E_1449;
									bx_715 = bx_675 - wLoc8E_1449;
									al_730 = es_719->*di_571;
									if (al_730 == 0x2D || (al_730 == 0x20 || al_730 == 0x2B))
									{
										si_741 = di_571 + 1;
										di_753 = fn0800_0B6E(SEQ(ds, di_745), SEQ(ss, fp - 2), es_719->*di_571, cx_713, wLoc8E_1449, bx_715);
										cx_757 = cx_713 - 0x01;
										bx_758 = bx_715 - 0x01;
									}
									cx_764 = wLoc8E_1449;
									if (wLoc8E_1449 != 0x00)
									{
										do
										{
											di_775 = fn0800_0B6E(SEQ(ds, di_767), SEQ(ss, fp - 2), 0x30, cx_773, cx_762, bx_788);
											cx_779 = cx_773 - 0x01;
										} while (cx_779 != 0x00);
									}
									dx_783 = cx_782;
									cx_784 = cx_762;
								}
								if (cx_785 != 0x00)
								{
									bx_789 = bx_786 - cx_785;
									do
									{
										ds->*di_797 = es_719->*si_792;
										v47_802 = bLoc57_1463 - 0x01;
										si_795 = si_792 + 1;
										di_799 = di_797 + 1;
										bLoc57_1464 = v47_802;
										if (v47_802 <= 0x00)
											di_809 = fn0800_0B76(SEQ(ss, fp - 2), cx_807, dx_1166, bx_789, di_799);
										cx_815 = cx_807 - 0x01;
									} while (cx_815 != 0x00);
								}
								bx_818 = bx_816;
								if (bx_816 > 0x00)
								{
									cx_823 = bx_816;
									do
									{
										di_834 = fn0800_0B6E(SEQ(ds, di_826), SEQ(ss, fp - 2), 0x20, cx_832, dx_1166, bx_816);
										cx_838 = cx_832 - 0x01;
									} while (cx_838 != 0x00);
								}
								goto l0800_0BAF;
							}
							if (wLoc8E_1441 <= 0x00)
								wLoc8E_1448 = 0x01;
						}
						goto l0800_0F8A;
					case 0x12:
						wArg04_1275 = si_101;
						if ((wLoc9A_1272 & 0x20) == 0x00)
						{
							di_112.u2 = (ds->*wArg02_1276).t0000.u2;
							sp_117 = sp_1222 - 2;
							(ss->*sp_117).u2 = (struct Eq_1338 *) ds;
							wArg02_1278 = (union Eq_6 Eq_6::*) &wArg02_1276->t0002.u0;
							es_di_1964 = (ptr32) di_112.u2;
						}
						else
						{
							es_di_108.u1 = (ds->*wArg02_1276).t0000.u1;
							wArg02_1277 = (union Eq_6 Eq_6::*) ((char *) &wArg02_1276->t0002 + 2);
						}
						*es_di_1965 = (uint16) (0x50 - bLoc57_1496);
						sp_1224 = sp_1222;
						goto l0800_0BAB;
					case 0x13:
					case 0x14:
					case 0x15:
						goto l0800_1035;
					case 22:
						wLoc9A_1274 = wLoc9A_1272 & ~0x20;
						ch_100 = 0x05;
						break;
					case 0x17:
						wLoc9A_1273 = wLoc9A_1272 | 0x20;
						ch_94 = 0x05;
						break;
					}
					al_1002 = ds->*si_101;
					si_1003 = si_101 + 1;
				}
l0800_0C4A:
l0800_1035:
				si_1013 = si_38;
				di_1014 = di_53;
				al_1015 = 0x25;
				do
				{
					di_1026 = fn0800_0B6E(SEQ(ds, di_1017), SEQ(ss, fp - 2), al_1022, cx_1182, dx_1199, bx_1178);
					al_1032 = ds->*si_1030;
					si_1033 = si_1030 + 1;
					al_1034 = al_1032;
				} while (al_1032 != 0x00);
				break;
			}
		}
		ds->*di_53 = al_1084;
		v18_1091 = bLoc57_1496 - 0x01;
		di_1088 = di_53 + 1;
		bLoc57_1536 = v18_1091;
		if (v18_1091 <= 0x00)
			di_1098 = fn0800_0B76(SEQ(ss, fp - 2), cx_1208, dx_1215, bx_1202, di_1088);
	}
	if (bLoc57_1496 < 0x50)
		fn0800_0B76(SEQ(ss, fp - 2), cx_1047, dx_1049, bx_1045, di_1051);
}

// 0800:0B61: Register word16 fn0800_0B61(Register (memptr (ptr16 Eq_638) byte) di, Register (ptr16 Eq_638) es)
// Called from:
//      __VPRINTER
word16 fn0800_0B61(byte Eq_638::* di, struct Eq_638 * es)
{
	word16 cx_13 = ~0x00;
	while (cx_13 != 0x00)
	{
		di = di_46 + 1;
		--cx_13;
		di_46 = di;
		if (es->*di_46 != 0x00)
			return -cx_13;
	}
	return -cx_13;
}

// 0800:0B6E: Register word16 fn0800_0B6E(Sequence (ptr32 byte) ds_di, Sequence (ptr32 Eq_246) ss_bp, Register byte al, Register Eq_21 cx, Register ci16 dx, Register Eq_21 bx)
// Called from:
//      __VPRINTER
word16 fn0800_0B6E(byte * ds_di, struct Eq_246 * ss_bp, byte al, Eq_21 cx, ci16 dx, Eq_21 bx)
{
	word16 di = (word16) ds_di;
	*ds_di = al;
	ci8 v10_11 = ss_bp->bFFFFFFAB - 0x01;
	ss_bp->bFFFFFFAB = v10_11;
	if (v10_11 <= 0x00)
		return di + 0x01;
	return fn0800_0B76(ss_bp, cx, dx, bx, di + 0x01);
}

// 0800:0B76: Register (memptr Eq_6 byte) fn0800_0B76(Sequence (ptr32 Eq_246) ss_bp, Register Eq_21 cx, Register ci16 dx, Register Eq_21 bx, Register (memptr Eq_6 byte) di)
// Called from:
//      __VPRINTER
//      fn0800_0B6E
byte Eq_6::* fn0800_0B76(struct Eq_246 * ss_bp, Eq_21 cx, ci16 dx, Eq_21 bx, byte Eq_6::* di)
{
	word16 bp = (word16) ss_bp;
	word16 di_20 = di - (bp - 84);
	word16 ax_34;
	(seg0800->*ss_bp->ptr000A)();
	if (ax_34 == 0x00)
		ss_bp->wFFFFFFFE = 0x01;
	ss_bp->bFFFFFFAB = 0x50;
	ss_bp->wFFFFFFA8 += di_20;
	return bp - 84;
}

