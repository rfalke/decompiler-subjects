...
// WARNING: Removing unreachable block (ram,0x001009a4)
...

{
  ...
  temp1 = num;
  for (temp2 = num / 2.0; temp2 * 1e-06 < temp1; temp2 = (num / temp2 + temp2) / 2.0) {
...

{
  ...
  while( true ) {
    if (*c_local != *d_local) {
      return (uint)(byte)*c_local - (uint)(byte)*d_local;
    }
    if (*c_local == '\0') break;
    c_local = c_local + 1;
    d_local = d_local + 1;
...

{
  ...
  i = 0;
  while (i = i + 1, i < 1000) {
    for (j = 0; buf[j] != '\0'; j = j + 1) {
      _IO_putc((uint)(byte)buf[j],__fp);
    }
  }
  fclose(__fp);
  return i;
