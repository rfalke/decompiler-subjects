// subject_bss.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

byte g_b411040 = 0x00; // 0000000000411040
// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 000000000040082C: void _fini()
void _fini()
{
}

// subject_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw410FE0 = 0x00; // 0000000000410FE0
// subject_got_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_ptr411000 = &g_t4004C0; // 0000000000411000
<anonymous> * g_ptr411008 = &g_t4004C0; // 0000000000411008
<anonymous> * g_ptr411010 = &g_t4004C0; // 0000000000411010
<anonymous> * g_ptr411018 = &g_t4004C0; // 0000000000411018
<anonymous> * g_ptr411020 = &g_t4004C0; // 0000000000411020
<anonymous> * g_ptr411028 = &g_t4004C0; // 0000000000411028
// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 00000000004004A0: Register word64 _init(Register out ptr64 x30Out)
// Called from:
//      __libc_csu_init
word64 _init(ptr64 & x30Out)
{
	call_weak_fn();
	x30Out = qwArg00;
	return x30;
}

// subject_init_array.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_a410DE0[] = // 0000000000410DE0
	{
	};
// subject_jcr.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw410DF0 = 0x00; // 0000000000410DF0
// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> g_t4004C0 = <code>; // 00000000004004C0
// 00000000004004E0: void fn00000000004004E0()
// Called from:
//      main
void fn00000000004004E0()
{
	g_ptr411000();
}

// 00000000004004F0: void fn00000000004004F0()
// Called from:
//      _start
void fn00000000004004F0()
{
	g_ptr411008();
}

// 0000000000400510: void fn0000000000400510()
// Called from:
//      _start
void fn0000000000400510()
{
	g_ptr411018();
}

// 0000000000400520: void fn0000000000400520()
// Called from:
//      main
void fn0000000000400520()
{
	g_ptr411020();
}

// 0000000000400530: void fn0000000000400530()
// Called from:
//      main
void fn0000000000400530()
{
	g_ptr411028();
}

// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400540: void _start()
void _start()
{
	fn00000000004004F0();
	fn0000000000400510();
}

// 0000000000400588: void call_weak_fn()
// Called from:
//      _init
void call_weak_fn()
{
	if (g_qw410FE0 == 0x00)
		return;
	word64 x17_12;
	g_ptr411010();
}

// 00000000004005A0: void deregister_tm_clones()
// Called from:
//      __do_global_dtors_aux
void deregister_tm_clones()
{
	if (0x00411047 - 0x00411040 <= 0x0E)
		return;
	<anonymous> * x1_12 = g_ptr4005D8;
	if (x1_12 == null)
		return;
	x1_12();
}

<anonymous> * g_ptr4005D8 = null; // 00000000004005D8
// 00000000004005E0: void register_tm_clones()
// Called from:
//      frame_dummy
void register_tm_clones()
{
	int64 x1_7 = 0x00411040 - 0x00411040;
	if ((x1_7 >> 3) + ((x1_7 >> 3) >>u 63) >> 1 == 0x00)
		return;
	<anonymous> * x2_12 = g_ptr400618;
	if (x2_12 == null)
		return;
	word64 x2_16;
	x2_12();
}

<anonymous> * g_ptr400618 = null; // 0000000000400618
// 0000000000400620: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if ((word32) g_b411040 == 0x00)
	{
		deregister_tm_clones();
		g_b411040 = 0x01;
	}
}

// 0000000000400650: void frame_dummy(Register word64 x29, Register word64 x30)
void frame_dummy(word64 x29, word64 x30)
{
	if (g_qw410DF0 != 0x00)
	{
		<anonymous> * x1_7 = g_ptr400688;
		if (x1_7 != null)
		{
			x1_7();
			register_tm_clones();
			return;
		}
	}
	register_tm_clones();
}

<anonymous> * g_ptr400688 = null; // 0000000000400688
// 0000000000400690: void main()
void main()
{
	fn0000000000400530();
	fn0000000000400520();
	struct Eq_95 * x29_28 = fp + -8;
	struct Eq_99 * sp_10 = fp - 0x38;
	while (true)
	{
		struct Eq_95 * x29_67;
		if (x29_28->dwFFFFFFF8 > x29_28->dwFFFFFFF4)
			break;
		fn0000000000400530();
		sp_10->dw0010 = (word32) 0x00400860;
		fn0000000000400520();
		int32 w8_60 = x29_28->dwFFFFFFF0;
		sp_10->dw000C = (word32) 0x0040085D;
		x29_67->dwFFFFFFEC = fib(w8_60, out x29_67);
		fn0000000000400530();
		sp_10 = (struct Eq_99 *) <invalid>;
		sp_10->dw0008 = (word32) 0x0040086F;
		++x29_67->dwFFFFFFF8;
		x29_28 = x29_67;
	}
	fn00000000004004E0();
	word64 x29_100;
	fib(0x00, out x29_100);
}

// 000000000040074C: Register int32 fib(Register int32 w0, Register out ptr64 x29Out)
// Called from:
//      main
//      fib
int32 fib(int32 w0, ptr64 & x29Out)
{
	struct Eq_161 * x29_31 = fp + -8;
	if (w0 > 0x02)
	{
		word64 x29_66;
		word32 w0_23 = fib(w0 - 0x01, out x29_66);
		struct Eq_99 * sp_20 = (struct Eq_99 *) <invalid>;
		word32 w8_26 = sp_20->dw0008;
		sp_20->dw0004 = w0_23;
		struct Eq_99 * sp_30 = (struct Eq_99 *) <invalid>;
		x29_31->dwFFFFFFFC = sp_30->dw0004 + fib(w8_26 - 0x02, out x29_31);
	}
	int32 w0_41 = x29_31->dwFFFFFFFC;
	x29Out = x29_31->ptr0000;
	return w0_41;
}

// 00000000004007B0: void __libc_csu_init(Register word32 w0, Register word64 x1, Register word64 x2, Register word64 x24)
void __libc_csu_init(word32 w0, word64 x1, word64 x2, word64 x24)
{
	word32 x24_32_32_104 = SLICE(x24, word32, 32);
	int64 x20_26 = 4263400 - 0x00410DE0;
	<anonymous> * x21_24[] = g_a410DE0;
	word64 x22_34 = x2;
	word64 x23_38 = x1;
	word64 x30_44;
	word64 x29_43 = _init(out x30_44);
	int64 x20_45 = x20_26 >> 3;
	if (x20_26 >> 3 != 0x00)
	{
		int64 x19_46 = 0x00;
		word64 x24_151 = SEQ(x24_32_32_104, w0);
		do
		{
			word64 x3_76;
			x21_24[x19_46]();
			int64 x31_81 = x20_45 - x19_46;
		} while (x31_81 != 0x00);
	}
}

// 0000000000400828: void __libc_csu_fini()
void __libc_csu_fini()
{
}

