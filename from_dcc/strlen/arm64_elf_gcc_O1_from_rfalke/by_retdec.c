//
// This file was generated by the Retargetable Decompiler
// Website: https://retdec.com
// Copyright (c) Retargetable Decompiler <info@retdec.com>
//

#include <stdint.h>
#include <stdlib.h>

// ------------------- Function Prototypes --------------------

int64_t __do_global_dtors_aux(void);
int64_t __libc_csu_fini(void);
int64_t __libc_csu_init(void);
int64_t _fini(int64_t a1);
int64_t _init(int64_t a1);
int64_t _start(int64_t a1);
int64_t call_weak_fn(void);
int64_t deregister_tm_clones(int64_t a1);
int64_t frame_dummy(void);
int64_t function_690(int64_t a1);
int32_t function_6a0(int64_t main2, int32_t argc, char ** ubp_av, void (*init)(), void (*fini)(), void (*rtld_fini)());
int64_t function_6b0(int64_t a1);
void function_6c0(void);
int64_t function_810(int64_t a1, int64_t a2);
int64_t register_tm_clones(int64_t * a1, int64_t a2);
int32_t strlen(char * s);

// --------------------- Global Variables ---------------------

int64_t g1 = 2016; // 0x10da8
int64_t g2 = 1944; // 0x10db0
int64_t g3 = 0; // 0x10db8
void (*g4)() = (void (*)())2240; // 0x10fa8
int64_t g5 = 0; // 0x10fb0
int64_t g6 = 0; // 0x10fc0
void (*g7)() = (void (*)())2120; // 0x10fc8
int64_t g8 = 2072; // 0x10fd0
int64_t g9 = 0; // 0x10fd8
int64_t g10 = 1648; // 0x11000
int64_t g11 = 0; // 0x11030
int32_t g12;

// ------------------------ Functions -------------------------

// Address range: 0x650 - 0x664
int64_t _init(int64_t a1) {
    // 0x650
    return call_weak_fn();
}

// Address range: 0x690 - 0x6a0
int64_t function_690(int64_t a1) {
    // 0x690
    return __cxa_finalize();
}

// Address range: 0x6a0 - 0x6b0
int32_t function_6a0(int64_t main2, int32_t argc, char ** ubp_av, void (*init)(), void (*fini)(), void (*rtld_fini)()) {
    // 0x6a0
    return __libc_start_main(main2, argc, ubp_av, init, fini, rtld_fini);
}

// Address range: 0x6b0 - 0x6c0
int64_t function_6b0(int64_t a1) {
    // 0x6b0
    return __gmon_start__();
}

// Address range: 0x6c0 - 0x6d0
void function_6c0(void) {
    // 0x6c0
    abort();
}

// Address range: 0x6d0 - 0x708
int64_t _start(int64_t a1) {
    // 0x6d0
    int64_t v1; // 0x6d0
    int64_t v2; // 0x6d0
    __libc_start_main(g8, (int32_t)a1, (char **)&v1, g7, g4, (void (*)())v2);
    abort();
    return &g12;
}

// Address range: 0x708 - 0x71c
int64_t call_weak_fn(void) {
    // 0x708
    if (g6 == 0) {
        // 0x718
        return 0;
    }
    // 0x714
    return function_6b0(g6);
}

// Address range: 0x720 - 0x754
int64_t deregister_tm_clones(int64_t a1) {
    if ((int64_t)&g11 + 7 - (int64_t)&g11 == 14 || (uint64_t)((int64_t)&g11 + 7 - (int64_t)&g11) < 14 || g5 == 0) {
        // 0x750
        return &g11;
    }
    // 0x74c
    return _ITM_deregisterTMCloneTable(&g11, g5);
}

// Address range: 0x758 - 0x794
int64_t register_tm_clones(int64_t * a1, int64_t a2) {
    // 0x758
    return &g11;
}

// Address range: 0x798 - 0x7e0
int64_t __do_global_dtors_aux(void) {
    unsigned char result = *(char *)&g11; // 0x7a8
    if (result != 0) {
        // 0x7d4
        return result;
    }
    int64_t v1 = 0; // 0x7b8
    if (*(int64_t *)0x10fb8 != 0) {
        // 0x7bc
        v1 = function_690(*(int64_t *)((int64_t)&g10 + 40));
    }
    // 0x7c8
    deregister_tm_clones(v1);
    *(char *)&g11 = 1;
    // 0x7d4
    return 1;
}

// Address range: 0x7e0 - 0x810
int64_t frame_dummy(void) {
    // 0x7e0
    if (g3 == 0 || g9 == 0) {
        // 0x7f0
        return register_tm_clones(&g3, 0);
    }
    // 0x804
    return _Jv_RegisterClasses(&g3, g9);
}

// Address range: 0x810 - 0x818
int64_t function_810(int64_t a1, int64_t a2) {
    // 0x810
    int64_t v1; // 0x810
    return register_tm_clones((int64_t *)v1, v1);
}

// From module:   /mnt/from_dcc/./strlen/source.c
// Address range: 0x818 - 0x820
// Line range:    1 - 5
int main() {
    // 0x818
    return 0;
}

// From module:   /mnt/from_dcc/./strlen/source.c
// Address range: 0x820 - 0x848
// Line range:    7 - 13
int32_t strlen(char * s) {
    // 0x820
    int64_t v1; // 0x820
    if ((char)v1 == 0) {
        // 0x840
        return 0;
    }
    int64_t v2 = (int64_t)s; // 0x82c
    v2++;
    int64_t result = 1; // 0x830
    int64_t v3 = result & 0xffffffff; // 0x838
    while (*(char *)v2 != 0) {
        // 0x830
        v2++;
        result = v3 + 1;
        v3 = result & 0xffffffff;
    }
    // 0x83c
    return result;
}

// Address range: 0x848 - 0x8c0
int64_t __libc_csu_init(void) {
    // 0x848
    int64_t v1; // 0x848
    int64_t result = _init(v1); // 0x87c
    if ((int64_t)&g2 - (int64_t)&g1 >> 3 == 0) {
        // 0x8ac
        return result;
    }
    // 0x88c
    return v1 & 0xffffffff;
}

// Address range: 0x8c0 - 0x8c4
int64_t __libc_csu_fini(void) {
    // 0x8c0
    int64_t result; // 0x8c0
    return result;
}

// Address range: 0x8c4 - 0x8d4
int64_t _fini(int64_t a1) {
    // 0x8c4
    int64_t result; // 0x8c4
    return result;
}

// --------------- Dynamically Linked Functions ---------------

// int64_t __cxa_finalize(void);
// int64_t __gmon_start__(void);
// int __libc_start_main(int *(main)(int, char **, char **), int argc, char ** ubp_av, void(* init)(void), void(* fini)(void), void(* rtld_fini)(void), void(* stack_end));
// int64_t _ITM_deregisterTMCloneTable(int64_t * a1, int64_t a2);
// int64_t _Jv_RegisterClasses(int64_t * a1, int64_t a2);
// void abort(void);

// --------------------- Meta-Information ---------------------

// Detected compiler/packer: gcc (6.3.0)
// Detected functions: 17

