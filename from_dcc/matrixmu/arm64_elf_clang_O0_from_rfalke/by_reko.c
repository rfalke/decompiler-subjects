// subject_bss.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

byte g_b411028 = 0x00; // 0000000000411028
// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400774: void _fini()
void _fini()
{
}

// subject_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw410FE0 = 0x00; // 0000000000410FE0
// subject_got_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_ptr411000 = &g_t4003F0; // 0000000000411000
<anonymous> * g_ptr411008 = &g_t4003F0; // 0000000000411008
<anonymous> * g_ptr411010 = &g_t4003F0; // 0000000000411010
// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 00000000004003D8: Register word64 _init(Register out ptr64 x30Out)
// Called from:
//      __libc_csu_init
word64 _init(ptr64 & x30Out)
{
	call_weak_fn();
	x30Out = qwArg00;
	return x30;
}

// subject_init_array.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_a410DE0[] = // 0000000000410DE0
	{
	};
// subject_jcr.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw410DF0 = 0x00; // 0000000000410DF0
// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> g_t4003F0 = <code>; // 00000000004003F0
// 0000000000400410: void fn0000000000400410()
// Called from:
//      _start
void fn0000000000400410()
{
	g_ptr411000();
}

// 0000000000400430: void fn0000000000400430()
// Called from:
//      _start
void fn0000000000400430()
{
	g_ptr411010();
}

// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400440: void _start()
void _start()
{
	fn0000000000400410();
	fn0000000000400430();
}

// 0000000000400488: void call_weak_fn()
// Called from:
//      _init
void call_weak_fn()
{
	if (g_qw410FE0 == 0x00)
		return;
	word64 x17_12;
	g_ptr411008();
}

// 00000000004004A0: void deregister_tm_clones()
// Called from:
//      __do_global_dtors_aux
void deregister_tm_clones()
{
	if (0x0041102F - 0x00411028 <= 0x0E)
		return;
	<anonymous> * x1_12 = g_ptr4004D8;
	if (x1_12 == null)
		return;
	x1_12();
}

<anonymous> * g_ptr4004D8 = null; // 00000000004004D8
// 00000000004004E0: void register_tm_clones()
// Called from:
//      frame_dummy
void register_tm_clones()
{
	int64 x1_7 = 0x00411028 - 0x00411028;
	if ((x1_7 >> 3) + ((x1_7 >> 3) >>u 63) >> 1 == 0x00)
		return;
	<anonymous> * x2_12 = g_ptr400518;
	if (x2_12 == null)
		return;
	word64 x2_16;
	x2_12();
}

<anonymous> * g_ptr400518 = null; // 0000000000400518
// 0000000000400520: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if ((word32) g_b411028 == 0x00)
	{
		deregister_tm_clones();
		g_b411028 = 0x01;
	}
}

// 0000000000400550: void frame_dummy(Register word64 x29, Register word64 x30)
void frame_dummy(word64 x29, word64 x30)
{
	if (g_qw410DF0 != 0x00)
	{
		<anonymous> * x1_7 = g_ptr400588;
		if (x1_7 != null)
		{
			x1_7();
			register_tm_clones();
			return;
		}
	}
	register_tm_clones();
}

<anonymous> * g_ptr400588 = null; // 0000000000400588
// 0000000000400590: void main()
void main()
{
	multMatrix(fp - 0x68, fp - 0xB8, fp - 0x0108);
}

// 00000000004005D0: void multMatrix(Register ptr64 x0, Register ptr64 x1, Register ptr64 x2)
// Called from:
//      main
void multMatrix(ptr64 x0, ptr64 x1, ptr64 x2)
{
	int32 dwLoc1C_91;
	for (dwLoc1C_91 = 0x00; dwLoc1C_91 < 0x05; ++dwLoc1C_91)
	{
		int32 dwLoc20_100;
		for (dwLoc20_100 = 0x00; dwLoc20_100 < 0x04; ++dwLoc20_100)
		{
			int32 dwLoc24_116 = 0x00;
			while (true)
			{
				uint16 wLoc28_106 = SLICE(dwLoc2A, word16, 16);
				int32 dwLoc26_104 = SEQ((word16) dwLoc24_116, wLoc26);
				if (dwLoc24_116 >= 0x04)
					break;
				(x2 + (int64) dwLoc26_104 * 0x14)[(int64) SEQ(wLoc26, wLoc28_106) *64 0] = (x0 + (int64) dwLoc26_104 * 0x00)[(int64) dwLoc2A *64 0] * (x1 + (int64) dwLoc2A * 0x14)[(int64) SEQ(wLoc26, wLoc28_106) *64 0] + (x2 + (int64) dwLoc26_104 * 0x14)[(int64) SEQ(wLoc26, wLoc28_106) *64 0];
				++dwLoc24_116;
			}
		}
	}
}

// 00000000004006F8: void __libc_csu_init(Register word32 w0, Register word64 x1, Register word64 x2, Register word64 x24)
void __libc_csu_init(word32 w0, word64 x1, word64 x2, word64 x24)
{
	word32 x24_32_32_104 = SLICE(x24, word32, 32);
	int64 x20_26 = 4263400 - 0x00410DE0;
	<anonymous> * x21_24[] = g_a410DE0;
	word64 x22_34 = x2;
	word64 x23_38 = x1;
	word64 x30_44;
	word64 x29_43 = _init(out x30_44);
	int64 x20_45 = x20_26 >> 3;
	if (x20_26 >> 3 != 0x00)
	{
		int64 x19_46 = 0x00;
		word64 x24_151 = SEQ(x24_32_32_104, w0);
		do
		{
			word64 x3_76;
			x21_24[x19_46]();
			int64 x31_81 = x20_45 - x19_46;
		} while (x31_81 != 0x00);
	}
}

// 0000000000400770: void __libc_csu_fini()
void __libc_csu_fini()
{
}

