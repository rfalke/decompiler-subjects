...
d8 time_based_random(void)
{
  ...
  return FLOAT_TO_SIGNED_INT(EXTRACT(reg_c, float_result), TRUNCATE_TO_ZERO) + (FLOAT_TO_SIGNED_INT(EXTRACT(reg_c, float_result), TRUNCATE_TO_ZERO) < 0 ? (FLOAT_TO_SIGNED_INT(EXTRACT(reg_c, float_result), TRUNCATE_TO_ZERO) + 0x3 & 0xfffffffffffffffc) * -1 : (FLOAT_TO_SIGNED_INT(EXTRACT(reg_c, float_result), TRUNCATE_TO_ZERO) & 0xfffffffffffffffc) * -1) + 6;
}
