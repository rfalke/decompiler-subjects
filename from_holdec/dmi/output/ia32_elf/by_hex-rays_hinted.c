/* This file was generated by the Hex-Rays decompiler.
   Copyright (c) 2007-2018 Hex-Rays <info@hex-rays.com>

   Detected compiler: GNU C++
*/

#include <defs.h>


//-------------------------------------------------------------------------
// Function declarations

void *init_proc();
int sub_8048410();
// int printf(const char *format, ...);
// time_t time(time_t *timer);
// int memcmp(const void *s1, const void *s2, size_t n);
// int fseek(FILE *stream, int off, int whence);
// size_t fwrite(const void *ptr, size_t size, size_t n, FILE *s);
// int puts(const char *s);
// char *strchr(const char *s, int c);
// int __cdecl __libc_start_main(int (__cdecl *main)(int, char **, char **), int argc, char **ubp_av, void (*init)(void), void (*fini)(void), void (*rtld_fini)(void), void *stack_end);
// FILE *fopen(const char *filename, const char *modes);
// int putchar(int c);
// int chmod(const char *file, __mode_t mode);
// int _gmon_start__(void); weak
int __cdecl main(int argc, const char **argv, const char **envp);
// void __usercall __noreturn start(int a1@<eax>, void (*a2)(void)@<edx>);
void _x86_get_pc_thunk_bx();
int deregister_tm_clones();
int register_tm_clones();
int _do_global_dtors_aux();
int frame_dummy();
int basic_1_string_literals_quoting();
int basic_2_string_literals_low_chars();
int basic_3_string_literals_high_chars();
int basic_4_readonly_vs_modifiable();
int __cdecl basic_10_result_var_name(signed int a1);
int __cdecl intermediate_1_for_loop_name(int a1);
int __cdecl intermediate_2_parameter_names(int a1);
int __cdecl intermediate_10_int_literal_in_bit_context(int a1);
int __cdecl intermediate_11_int_literal_in_arithm_context(int a1);
int __cdecl advanced_1_null_bytes_with_fixed_length(FILE *s); // idb
int __cdecl advanced_2_naming_enums(FILE *stream); // idb
int __cdecl advanced_10_int_literal_in_char_context(char *s); // idb
int advanced_11_printf_type_related();
void _libc_csu_init(void); // idb
void _libc_csu_fini(void); // idb
void term_proc();
// size_t fwrite(const void *ptr, size_t size, size_t n, FILE *s);

//-------------------------------------------------------------------------
// Data declarations

int (*_frame_dummy_init_array_entry[2])() = { &frame_dummy, &_do_global_dtors_aux }; // weak
int (*_do_global_dtors_aux_fini_array_entry)() = &_do_global_dtors_aux; // weak
int (*dword_804B008)(void) = NULL; // weak
char a_modifiable_string[] = "a modifiable string"; // idb
char _bss_start; // weak
_UNKNOWN unk_804B053; // weak
// extern _UNKNOWN __gmon_start__; weak


//----- (080483E8) --------------------------------------------------------
void *init_proc()
{
  void *result; // eax

  result = &__gmon_start__;
  if ( &__gmon_start__ )
    result = (void *)_gmon_start__();
  return result;
}
// 80484D0: using guessed type int _gmon_start__(void);

//----- (08048410) --------------------------------------------------------
int sub_8048410()
{
  return dword_804B008();
}
// 804B008: using guessed type int (*dword_804B008)(void);

//----- (080484E0) --------------------------------------------------------
int __cdecl main(int argc, const char **argv, const char **envp)
{
  basic_1_string_literals_quoting();
  basic_2_string_literals_low_chars();
  basic_3_string_literals_high_chars();
  basic_4_readonly_vs_modifiable();
  basic_10_result_var_name(argc);
  intermediate_1_for_loop_name(argc);
  intermediate_2_parameter_names(argc);
  intermediate_10_int_literal_in_bit_context(argc);
  intermediate_11_int_literal_in_arithm_context(argc);
  advanced_1_null_bytes_with_fixed_length(0);
  advanced_2_naming_enums(0);
  advanced_10_int_literal_in_char_context((char *)*argv);
  advanced_11_printf_type_related();
  return **argv;
}

//----- (0804856D) --------------------------------------------------------
// positive sp value has been detected, the output may be wrong!
void __usercall __noreturn start(int a1@<eax>, void (*a2)(void)@<edx>)
{
  int v2; // esi
  int v3; // [esp-4h] [ebp-4h]
  int _0; // [esp+0h] [ebp+0h]

  v2 = v3;
  v3 = a1;
  __libc_start_main(
    (int (__cdecl *)(int, char **, char **))main,
    v2,
    (char **)&_0,
    _libc_csu_init,
    _libc_csu_fini,
    a2,
    &v3);
  __halt();
}
// 8048570: positive sp value 4 has been found

//----- (08048590) --------------------------------------------------------
void _x86_get_pc_thunk_bx()
{
  ;
}

//----- (080485A0) --------------------------------------------------------
int deregister_tm_clones()
{
  int result; // eax

  result = &unk_804B053 - (_UNKNOWN *)&_bss_start;
  if ( (unsigned int)(&unk_804B053 - (_UNKNOWN *)&_bss_start) > 6 )
    result = 0;
  return result;
}
// 80485A0: could not find valid save-restore pair for ebp
// 804B050: using guessed type char _bss_start;

//----- (080485D0) --------------------------------------------------------
int register_tm_clones()
{
  return 0;
}
// 80485D0: could not find valid save-restore pair for ebp

//----- (08048610) --------------------------------------------------------
int _do_global_dtors_aux()
{
  int result; // eax

  if ( !_bss_start )
  {
    result = deregister_tm_clones();
    _bss_start = 1;
  }
  return result;
}
// 8048610: could not find valid save-restore pair for ebp
// 804B050: using guessed type char _bss_start;

//----- (08048630) --------------------------------------------------------
int frame_dummy()
{
  return register_tm_clones();
}
// 8048630: could not find valid save-restore pair for ebp

//----- (08048660) --------------------------------------------------------
int basic_1_string_literals_quoting()
{
  puts("with ' single quote");
  puts("with \" double quote");
  return 0;
}

//----- (08048680) --------------------------------------------------------
int basic_2_string_literals_low_chars()
{
  // since \1 and similar characters are unprintable, ida truncated
  // string constants on them. i manually extended all strings
  puts("hex=0x01 dec=1 char=\x01FINI");
  puts("hex=0x02 dec=2 char=\x02FINI");
  puts("hex=0x03 dec=3 char=\x03FINI");
  puts("hex=0x04 dec=4 char=\x04FINI");
  puts("hex=0x05 dec=5 char=\x05FINI");
  puts("hex=0x06 dec=6 char=\x06FINI");
  puts("hex=0x07 dec=7 char=\aFINI");
  puts("hex=0x08 dec=8 char=\bFINI");
  puts("hex=0x09 dec=9 char=\tFINI");
  puts("hex=0x0a dec=10 char=\nFINI");
  puts("hex=0x0b dec=11 char=\vFINI");
  puts("hex=0x0c dec=12 char=\fFINI");
  puts("hex=0x0d dec=13 char=\rFINI");
  puts("hex=0x0e dec=14 char=\x0EFINI");
  puts("hex=0x0f dec=15 char=\x0FFINI");
  puts("hex=0x10 dec=16 char=\x10FINI");
  puts("hex=0x11 dec=17 char=\x11FINI");
  puts("hex=0x12 dec=18 char=\x12FINI");
  puts("hex=0x13 dec=19 char=\x13FINI");
  puts("hex=0x14 dec=20 char=\x14FINI");
  puts("hex=0x15 dec=21 char=\x15FINI");
  puts("hex=0x16 dec=22 char=\x16FINI");
  puts("hex=0x17 dec=23 char=\x17FINI");
  puts("hex=0x18 dec=24 char=\x18FINI");
  puts("hex=0x19 dec=25 char=\x19FINI");
  puts("hex=0x1a dec=26 char=\x1AFINI");
  puts("hex=0x1b dec=27 char=\x1BFINI");
  puts("hex=0x1c dec=28 char=\x1CFINI");
  puts("hex=0x1d dec=29 char=\x1DFINI");
  puts("hex=0x1e dec=30 char=\x1EFINI");
  puts("hex=0x1f dec=31 char=\x1FFINI");
  return 0;
}

//----- (08048800) --------------------------------------------------------
int basic_3_string_literals_high_chars()
{
  // fixme: i set the codepage for the string as CP1252 but the
  // output does not look good
  puts("hex=0xf0 dec=240 char=ðFINI");
  puts("hex=0xf1 dec=241 char=ñFINI");
  puts("hex=0xf2 dec=242 char=òFINI");
  puts("hex=0xf3 dec=243 char=óFINI");
  puts("hex=0xf4 dec=244 char=ôFINI");
  puts("hex=0xf5 dec=245 char=õFINI");
  puts("hex=0xf6 dec=246 char=öFINI");
  puts("hex=0xf7 dec=247 char=÷FINI");
  puts("hex=0xf8 dec=248 char=øFINI");
  puts("hex=0xf9 dec=249 char=ùFINI");
  puts("hex=0xfa dec=250 char=úFINI");
  puts("hex=0xfb dec=251 char=ûFINI");
  puts("hex=0xfc dec=252 char=üFINI");
  puts("hex=0xfd dec=253 char=ýFINI");
  puts("hex=0xfe dec=254 char=þFINI");
  puts("hex=0xff dec=255 char=\xFFFINI");
  return 0;
}

//----- (080488D0) --------------------------------------------------------
int basic_4_readonly_vs_modifiable()
{
  puts("a read only string");
  puts(a_modifiable_string);
  return 0;
}

//----- (080488F0) --------------------------------------------------------
int __cdecl basic_10_result_var_name(signed int a1)
{
  int result; // eax

  if ( a1 <= 5 )
    result = time(0) + 10;
  else
    result = 4 * a1;
  return result;
}

//----- (08048920) --------------------------------------------------------
int __cdecl intermediate_1_for_loop_name(int a1)
{
  signed int v1; // ebx
  int v3; // [esp-10h] [ebp-1Ch]

  // fixme: recognize for-loop
  if ( a1 > 0 )
  {
    v1 = 48;
    do
    {
      v3 = v1++;
      putchar(v3);
    }
    while ( v1 != a1 + 48 );
  }
  return 0;
}

//----- (08048960) --------------------------------------------------------
int __cdecl intermediate_2_parameter_names(int a1)
{
  const char *mode; // eax
  const char *path; // edx

  if ( a1 == 1 )
  {
    path = "/is/a/1";
    mode = "r";
  }
  else
  {
    mode = "r";
    path = "/is/something/else";
    if ( a1 < 0 )
      mode = "w";
  }
  if ( !fopen(path, mode) )
    puts("failed");
  return 0;
}

//----- (080489D0) --------------------------------------------------------
int __cdecl intermediate_10_int_literal_in_bit_context(int a1)
{
  putchar(a1 & 0xF0F0);
  putchar(a1 | 0xFF0000);
  putchar(a1 ^ 3);
  putchar(a1 & 42);
  putchar(a1 | 999);
  putchar(a1 ^ 1234321);
  return 0;
}

//----- (08048A40) --------------------------------------------------------
int __cdecl intermediate_11_int_literal_in_arithm_context(int a1)
{
  putchar(a1 + 0xF0F0);
  putchar(a1 - 0xFF0000);
  putchar(3 * a1);
  putchar(a1 + 42);
  putchar(a1 - 999);
  putchar(1234321 * a1);
  return 0;
}

//----- (08048AA0) --------------------------------------------------------
int __cdecl advanced_1_null_bytes_with_fixed_length(FILE *s)
{
  // fixme: show the embedded zero byte
  fwrite("with null byte ", 0x11u, 1u, s);
  fwrite("with null byte ", 1u, 0x11u, s);
  return memcmp(s, "with null byte ", 0x11u);
}

//----- (08048AE0) --------------------------------------------------------
int __cdecl advanced_2_naming_enums(FILE *stream)
{
  int v3; // ebx MAPDST

  v3 = fseek(stream, 0, SEEK_SET);
  v3 += fseek(stream, 0, SEEK_CUR);
  v3 += fseek(stream, 0, SEEK_END);
  if ( v3 + chmod("/some/path", S_IRGRP|S_IROTH) > 4 )
    puts("something");
  return 0;
}

//----- (08048B50) --------------------------------------------------------
int __cdecl advanced_10_int_literal_in_char_context(char *s)
{
  char arr[3]; // [esp+19h] [ebp-Fh]

  putchar('1');
  if ( strchr(s, '1') )
    puts("contains a '1'");
  strcpy(arr, "a1");
  puts(arr);
  return 0;
}

//----- (08048BA0) --------------------------------------------------------
int advanced_11_printf_type_related()
{
  // fixme: why not all variadic arguments got recognized correctly?
  printf("as char %c\n", '0');
  printf("as int %d\n", 48);                    // used "Add variadic argument"
  printf("as pointer %p\n", 48);
  printf("as octal %o\n", 060);                 // used "Add variadic argument"
  printf("as hex %x\n", 0x30);
  printf("as unsigned %u\n", 4294967295);       // used "Add variadic argument"
  printf("as signed %d\n", -1);
  return 0;
}

//----- (08048C10) --------------------------------------------------------
void _libc_csu_init(void)
{
  int v0; // esi
  int i; // edi

  init_proc();
  v0 = &_do_global_dtors_aux_fini_array_entry - _frame_dummy_init_array_entry;
  if ( v0 )
  {
    for ( i = 0; i != v0; ++i )
      _frame_dummy_init_array_entry[i]();
  }
}
// 804AF08: using guessed type int (*_frame_dummy_init_array_entry[2])();
// 804AF0C: using guessed type int (*_do_global_dtors_aux_fini_array_entry)();

//----- (08048C74) --------------------------------------------------------
void term_proc()
{
  ;
}

// ALL OK, 24 function(s) have been successfully decompiled
