// subject_bss.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

byte g_b804B050 = 0x00; // 0804B050
// subject_data.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

char g_b804B03C = 'a'; // 0804B03C
// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 08048C74: void _fini()
void _fini()
{
}

// subject_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_ptr804AFFC = null; // 0804AFFC
// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080483E8: void _init()
// Called from:
//      __libc_csu_init
void _init()
{
	if (g_ptr804AFFC != null)
		fn080484D0();
}

// subject_init_array.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_a804AF08[] = // 0804AF08
	{
	};
// subject_jcr.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word32 g_dw804AF10 = 0x00; // 0804AF10
// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 08048420: void printf()
void printf()
{
}

// 08048430: void time()
void time()
{
}

// 08048440: void memcmp()
void memcmp()
{
}

// 08048450: void fseek()
void fseek()
{
}

// 08048460: void fwrite()
void fwrite()
{
}

// 08048470: void puts()
void puts()
{
}

// 08048480: void strchr()
void strchr()
{
}

// 08048490: void __libc_start_main()
void __libc_start_main()
{
}

// 080484A0: void fopen()
void fopen()
{
}

// 080484B0: void putchar()
void putchar()
{
}

// 080484C0: void chmod()
void chmod()
{
}

// subject_plt_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080484D0: void fn080484D0()
// Called from:
//      _init
void fn080484D0()
{
	g_ptr804AFFC();
}

// subject_rodata.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word32 _fp_hw = 0x03; // 08048C88
word32 _IO_stdin_used = 0x00020001; // 08048C8C
char g_str8048C94[] = "with ' single quote"; // 08048C94
char g_str8048CA8[] = "with \" double quote"; // 08048CA8
char g_str8048CBC[] = "hex=0x01 dec=1 char=\x01FINI"; // 08048CBC
char g_str8048CD6[] = "hex=0x02 dec=2 char=\x02FINI"; // 08048CD6
char g_str8048CF0[] = "hex=0x03 dec=3 char=\x03FINI"; // 08048CF0
char g_str8048D0A[] = "hex=0x04 dec=4 char=\x04FINI"; // 08048D0A
char g_str8048D24[] = "hex=0x05 dec=5 char=\x05FINI"; // 08048D24
char g_str8048D3E[] = "hex=0x06 dec=6 char=\x06FINI"; // 08048D3E
char g_str8048D58[] = "hex=0x07 dec=7 char=\aFINI"; // 08048D58
char g_str8048D72[] = "hex=0x08 dec=8 char=\bFINI"; // 08048D72
char g_str8048D8C[] = "hex=0x09 dec=9 char=\tFINI"; // 08048D8C
char g_str8048DA6[] = "hex=0x0a dec=10 char=\nFINI"; // 08048DA6
char g_str8048DC1[] = "hex=0x0b dec=11 char=\vFINI"; // 08048DC1
char g_str8048DDC[] = "hex=0x0c dec=12 char=\fFINI"; // 08048DDC
char g_str8048DF7[] = "hex=0x0d dec=13 char=\rFINI"; // 08048DF7
char g_str8048E12[] = "hex=0x0e dec=14 char=\x0EFINI"; // 08048E12
char g_str8048E2D[] = "hex=0x0f dec=15 char=\x0FFINI"; // 08048E2D
char g_str8048E48[] = "hex=0x10 dec=16 char=\x10FINI"; // 08048E48
char g_str8048E63[] = "hex=0x11 dec=17 char=\x11FINI"; // 08048E63
char g_str8048E7E[] = "hex=0x12 dec=18 char=\x12FINI"; // 08048E7E
char g_str8048E99[] = "hex=0x13 dec=19 char=\x13FINI"; // 08048E99
char g_str8048EB4[] = "hex=0x14 dec=20 char=\x14FINI"; // 08048EB4
char g_str8048ECF[] = "hex=0x15 dec=21 char=\x15FINI"; // 08048ECF
char g_str8048EEA[] = "hex=0x16 dec=22 char=\x16FINI"; // 08048EEA
char g_str8048F05[] = "hex=0x17 dec=23 char=\x17FINI"; // 08048F05
char g_str8048F20[] = "hex=0x18 dec=24 char=\x18FINI"; // 08048F20
char g_str8048F3B[] = "hex=0x19 dec=25 char=\x19FINI"; // 08048F3B
char g_str8048F56[] = "hex=0x1a dec=26 char=\x1AFINI"; // 08048F56
char g_str8048F71[] = "hex=0x1b dec=27 char=\x1BFINI"; // 08048F71
char g_str8048F8C[] = "hex=0x1c dec=28 char=\x1CFINI"; // 08048F8C
char g_str8048FA7[] = "hex=0x1d dec=29 char=\x1DFINI"; // 08048FA7
char g_str8048FC2[] = "hex=0x1e dec=30 char=\x1EFINI"; // 08048FC2
char g_str8048FDD[] = "hex=0x1f dec=31 char=\x1FFINI"; // 08048FDD
char g_str8048FF8[] = "hex=0xf0 dec=240 char=\xF0FINI"; // 08048FF8
char g_str8049014[] = "hex=0xf1 dec=241 char=\xF1FINI"; // 08049014
char g_str8049030[] = "hex=0xf2 dec=242 char=\xF2FINI"; // 08049030
char g_str804904C[] = "hex=0xf3 dec=243 char=\xF3FINI"; // 0804904C
char g_str8049068[] = "hex=0xf4 dec=244 char=\xF4FINI"; // 08049068
char g_str8049084[] = "hex=0xf5 dec=245 char=\xF5FINI"; // 08049084
char g_str80490A0[] = "hex=0xf6 dec=246 char=\xF6FINI"; // 080490A0
char g_str80490BC[] = "hex=0xf7 dec=247 char=\xF7FINI"; // 080490BC
char g_str80490D8[] = "hex=0xf8 dec=248 char=\xF8FINI"; // 080490D8
char g_str80490F4[] = "hex=0xf9 dec=249 char=\xF9FINI"; // 080490F4
char g_str8049110[] = "hex=0xfa dec=250 char=\xFAFINI"; // 08049110
char g_str804912C[] = "hex=0xfb dec=251 char=\xFBFINI"; // 0804912C
char g_str8049148[] = "hex=0xfc dec=252 char=\xFCFINI"; // 08049148
char g_str8049164[] = "hex=0xfd dec=253 char=\xFDFINI"; // 08049164
char g_str8049180[] = "hex=0xfe dec=254 char=\xFEFINI"; // 08049180
char g_str804919C[] = "hex=0xff dec=255 char=\xFFFINI"; // 0804919C
char g_str80491B8[] = "a read only string"; // 080491B8
char g_str80491CB[] = "/is/something/else"; // 080491CB
char g_str80491DE[] = "r"; // 080491DE
char g_str80491E0[] = "w"; // 080491E0
char g_str80491E2[] = "/is/a/1"; // 080491E2
char g_str80491EA[] = "failed"; // 080491EA
char g_str80491F1[] = "/some/path"; // 080491F1
char g_str80491FC[] = "something"; // 080491FC
char g_str8049206[] = "contains a '1'"; // 08049206
char g_str8049215[] = "as char %c\n"; // 08049215
char g_str8049221[] = "as int %d\n"; // 08049221
char g_str804922C[] = "as pointer %p\n"; // 0804922C
char g_str804923B[] = "as octal %o\n"; // 0804923B
char g_str8049248[] = "as hex %x\n"; // 08049248
char g_str8049253[] = "as unsigned %u\n"; // 08049253
char g_str8049263[] = "as signed %d\n"; // 08049263
void g_v8049271 = ??void??; // 08049271
// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080484E0: void main(Stack int32 dwArg04, Stack (ptr32 (ptr32 char)) dwArg08)
void main(int32 dwArg04, char ** dwArg08)
{
	char ** fp;
	__align_stack<word32>(fp);
	basic_1_string_literals_quoting();
	basic_2_string_literals_low_chars();
	basic_3_string_literals_high_chars();
	basic_4_readonly_vs_modifiable();
	basic_10_result_var_name(dwArg04);
	intermediate_1_for_loop_name(dwArg04);
	intermediate_2_parameter_names(dwArg04);
	intermediate_10_int_literal_in_bit_context(dwArg04);
	intermediate_11_int_literal_in_arithm_context(dwArg04);
	advanced_1_null_bytes_with_fixed_length(null);
	advanced_2_naming_enums(null);
	advanced_10_int_literal_in_char_context(*dwArg08);
	advanced_11_printf_type_related();
}

// 0804856D: void _start(Register (ptr32 Eq_69) edx, Stack int32 dwArg00, Stack (ptr32 char) ptrArg04)
void _start(void (* edx)(), int32 dwArg00, char * ptrArg04)
{
	void * fp;
	__align_stack<word32>(&ptrArg04);
	__libc_start_main(&g_t80484E0, dwArg00, &ptrArg04, &g_t8048C10, &g_t8048C70, edx, fp);
	__halt();
}

// 08048590: void __x86.get_pc_thunk.bx()
void __x86.get_pc_thunk.bx()
{
}

// 080485A0: void deregister_tm_clones()
// Called from:
//      __do_global_dtors_aux
void deregister_tm_clones()
{
}

// 080485D0: void register_tm_clones()
// Called from:
//      frame_dummy
void register_tm_clones()
{
}

// 08048610: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if (g_b804B050 == 0x00)
	{
		deregister_tm_clones();
		g_b804B050 = 0x01;
	}
}

// 08048630: void frame_dummy()
void frame_dummy()
{
	register_tm_clones();
}

// 08048660: void basic_1_string_literals_quoting()
// Called from:
//      main
void basic_1_string_literals_quoting()
{
	puts("with ' single quote");
	puts("with \" double quote");
}

// 08048680: void basic_2_string_literals_low_chars()
// Called from:
//      main
void basic_2_string_literals_low_chars()
{
	puts("hex=0x01 dec=1 char=\x01FINI");
	puts("hex=0x02 dec=2 char=\x02FINI");
	puts("hex=0x03 dec=3 char=\x03FINI");
	puts("hex=0x04 dec=4 char=\x04FINI");
	puts("hex=0x05 dec=5 char=\x05FINI");
	puts("hex=0x06 dec=6 char=\x06FINI");
	puts("hex=0x07 dec=7 char=\aFINI");
	puts("hex=0x08 dec=8 char=\bFINI");
	puts("hex=0x09 dec=9 char=\tFINI");
	puts("hex=0x0a dec=10 char=\nFINI");
	puts("hex=0x0b dec=11 char=\vFINI");
	puts("hex=0x0c dec=12 char=\fFINI");
	puts("hex=0x0d dec=13 char=\rFINI");
	puts("hex=0x0e dec=14 char=\x0EFINI");
	puts("hex=0x0f dec=15 char=\x0FFINI");
	puts("hex=0x10 dec=16 char=\x10FINI");
	puts("hex=0x11 dec=17 char=\x11FINI");
	puts("hex=0x12 dec=18 char=\x12FINI");
	puts("hex=0x13 dec=19 char=\x13FINI");
	puts("hex=0x14 dec=20 char=\x14FINI");
	puts("hex=0x15 dec=21 char=\x15FINI");
	puts("hex=0x16 dec=22 char=\x16FINI");
	puts("hex=0x17 dec=23 char=\x17FINI");
	puts("hex=0x18 dec=24 char=\x18FINI");
	puts("hex=0x19 dec=25 char=\x19FINI");
	puts("hex=0x1a dec=26 char=\x1AFINI");
	puts("hex=0x1b dec=27 char=\x1BFINI");
	puts("hex=0x1c dec=28 char=\x1CFINI");
	puts("hex=0x1d dec=29 char=\x1DFINI");
	puts("hex=0x1e dec=30 char=\x1EFINI");
	puts("hex=0x1f dec=31 char=\x1FFINI");
}

// 08048800: void basic_3_string_literals_high_chars()
// Called from:
//      main
void basic_3_string_literals_high_chars()
{
	puts("hex=0xf0 dec=240 char=\xF0FINI");
	puts("hex=0xf1 dec=241 char=\xF1FINI");
	puts("hex=0xf2 dec=242 char=\xF2FINI");
	puts("hex=0xf3 dec=243 char=\xF3FINI");
	puts("hex=0xf4 dec=244 char=\xF4FINI");
	puts("hex=0xf5 dec=245 char=\xF5FINI");
	puts("hex=0xf6 dec=246 char=\xF6FINI");
	puts("hex=0xf7 dec=247 char=\xF7FINI");
	puts("hex=0xf8 dec=248 char=\xF8FINI");
	puts("hex=0xf9 dec=249 char=\xF9FINI");
	puts("hex=0xfa dec=250 char=\xFAFINI");
	puts("hex=0xfb dec=251 char=\xFBFINI");
	puts("hex=0xfc dec=252 char=\xFCFINI");
	puts("hex=0xfd dec=253 char=\xFDFINI");
	puts("hex=0xfe dec=254 char=\xFEFINI");
	puts("hex=0xff dec=255 char=\xFFFINI");
}

// 080488D0: void basic_4_readonly_vs_modifiable()
// Called from:
//      main
void basic_4_readonly_vs_modifiable()
{
	puts("a read only string");
	puts(&g_b804B03C);
}

// 080488F0: void basic_10_result_var_name(Stack int32 dwArg04)
// Called from:
//      main
void basic_10_result_var_name(int32 dwArg04)
{
	if (dwArg04 > 0x05)
		return;
	time(null);
}

// 08048920: void intermediate_1_for_loop_name(Stack int32 dwArg04)
// Called from:
//      main
void intermediate_1_for_loop_name(int32 dwArg04)
{
	if (dwArg04 > 0x00)
	{
		int32 ebx_18 = 0x30;
		do
		{
			putchar(ebx_18);
			++ebx_18;
		} while (ebx_18 != dwArg04 + 48);
	}
}

// 08048960: void intermediate_2_parameter_names(Stack int32 dwArg04)
// Called from:
//      main
void intermediate_2_parameter_names(int32 dwArg04)
{
	char * edx_16;
	char * eax_10;
	if (dwArg04 != 0x01)
	{
		eax_10 = (char *) "r";
		edx_16 = (char *) "/is/something/else";
		if (dwArg04 < 0x00)
			eax_10 = (char *) "w";
	}
	else
	{
		edx_16 = (char *) "/is/a/1";
		eax_10 = (char *) "r";
	}
	if (fopen(edx_16, eax_10) != null)
		return;
	puts("failed");
}

// 080489D0: void intermediate_10_int_literal_in_bit_context(Stack int32 dwArg04)
// Called from:
//      main
void intermediate_10_int_literal_in_bit_context(int32 dwArg04)
{
	putchar(dwArg04 & 0xF0F0);
	putchar(dwArg04 | 0x00FF0000);
	putchar(dwArg04 ^ 0x03);
	putchar(dwArg04 & 0x2A);
	putchar(dwArg04 | 999);
	putchar(dwArg04 ^ 1234321);
}

// 08048A40: void intermediate_11_int_literal_in_arithm_context(Stack int32 dwArg04)
// Called from:
//      main
void intermediate_11_int_literal_in_arithm_context(int32 dwArg04)
{
	putchar(dwArg04 + 0xF0F0);
	putchar(dwArg04 + 0xFF010000);
	putchar(dwArg04 * 0x03);
	putchar(dwArg04 + 42);
	putchar(dwArg04 - 999);
	putchar(dwArg04 *s 1234321);
}

// 08048AA0: void advanced_1_null_bytes_with_fixed_length(Stack (ptr32 Eq_51) dwArg04)
// Called from:
//      main
void advanced_1_null_bytes_with_fixed_length(FILE * dwArg04)
{
	fwrite(&g_v8049271, 0x11, 0x01, dwArg04);
	fwrite(&g_v8049271, 0x01, 0x11, dwArg04);
	memcmp(dwArg04, &g_v8049271, 0x11);
}

// 08048AE0: void advanced_2_naming_enums(Stack (ptr32 Eq_56) dwArg04)
// Called from:
//      main
void advanced_2_naming_enums(FILE * dwArg04)
{
	if (chmod("/some/path", 0x24) + ((fseek(dwArg04, 0x00, 0x00) + fseek(dwArg04, 0x00, 0x01)) + fseek(dwArg04, 0x00, 0x02)) > 0x04)
		puts("something");
}

// 08048B50: void advanced_10_int_literal_in_char_context(Stack (ptr32 char) dwArg04)
// Called from:
//      main
void advanced_10_int_literal_in_char_context(char * dwArg04)
{
	char bLoc0F;
	putchar(0x31);
	if (strchr(dwArg04, '1') != null)
		puts("contains a '1'");
	bLoc0F = 0x61;
	puts(&bLoc0F);
}

// 08048BA0: void advanced_11_printf_type_related()
// Called from:
//      main
void advanced_11_printf_type_related()
{
	printf("as char %c\n", '0');
	printf("as int %d\n", 0x30);
	printf("as pointer %p\n", 0x30);
	printf("as octal %o\n", 0x30);
	printf("as hex %x\n", 0x30);
	printf("as unsigned %u\n", ~0x00);
	printf("as signed %d\n", ~0x00);
}

// 08048C10: void __libc_csu_init(Stack word32 dwArg04, Stack word32 dwArg08, Stack word32 dwArg0C)
void __libc_csu_init(word32 dwArg04, word32 dwArg08, word32 dwArg0C)
{
	_init();
	int32 esi_29 = 0x0804AF0C - g_a804AF08;
	if (esi_29 >> 0x02 != 0x00)
	{
		int32 edi_35 = 0x00;
		do
		{
			(*((char *) g_a804AF08 + edi_35 * 0x04))();
			++edi_35;
		} while (esi_29 >> 0x02 != edi_35);
	}
}

// 08048C70: void __libc_csu_fini()
void __libc_csu_fini()
{
}

