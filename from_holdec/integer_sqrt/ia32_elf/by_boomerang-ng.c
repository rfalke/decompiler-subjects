...
void integer_sqrt_in_c(unsigned int param1)
{
    ...
    unsigned char cl; 		// r9
    unsigned int cx; 		// r1
    unsigned int cx_1; 		// r1{0}
    int di; 		// r7
    unsigned short di_1; 		// r7{0}
    int di_6; 		// r7{0}
    unsigned char dl; 		// r10
    unsigned int dx; 		// r2
    unsigned int dx_1; 		// r2{0}
    ...
    unsigned int local4; 		// m[esp - 20]{0}
    unsigned int local5; 		// m[esp - 24]{0}
    unsigned int local6; 		// dx{0}
    unsigned int local7; 		// ebx_1{0}
    unsigned int si; 		// r6
    ...
    for(;;) {
        ...
        local4 = local2;
        local0 = local5 + 1;
        eax = eax + eax + 1 >> 16 & 0xffff | (local5 - si);
        tmp1 = local0 - 16;
        if (local0 == 16) {
            break;
        }
        cl =  ((unsigned int)bx > (unsigned short) ebx_1 + ebx_1) ? 1 : 0;
        edi = ecx >> 8 & 0xffffff | (cl);
        ecx_1 = (local3);
        di_6 = (unsigned short) edx + (edi >> 16 & 0xffff | ((unsigned short) edi & 0xff));
        dl =  (ADDFLAGS16(dx, di_6, tmp1 + di_6)) ? 1 : 0;
        edx = (edx >> 16 & 0xffff | (tmp1 + di_6)) >> 8 & 0xffffff | (dl);
        di_1 = (((unsigned short) edx & 0xff) + local3);
        edi = (edx >> 16 & 0xffff | ((unsigned short) edx & 0xff)) >> 16 & 0xffff | (di_1);
        ebx_2 = ebx_1 * 4;
        bx = (unsigned short) ebx_1 * 4;
        dl =  ((unsigned short) ebx_1 + ebx_1 > (unsigned short) ebx_1 * 4) ? 1 : 0;
        si = (((unsigned short) (edx >> 8 & 0xffffff | (dl)) & 0xff) + tmp1 + di_6);
        dx = ecx_1 + si;
        edx = (tmp1 + di_6) >> 16 & 0xffff | (ecx_1 + si);
        cl =  (ADDFLAGS16((unsigned short) (tmp1 + di_6), si, ecx_1 + si)) ? 1 : 0;
        ecx_4 = ecx_1 + edi >> 8 & 0xffffff | (cl);
        si = (((unsigned short) ecx_4 & 0xff) + (unsigned short) ecx_1 + edi);
        cl =  ((unsigned short) ecx_1 + edi < di_1) ? 1 : 0;
        esi = ecx_4 >> 8 & 0xffffff | (cl);
        ecx = ((unsigned short) ecx_1 + edi);
        cx_1 = ((unsigned short) ecx + si);
        local2 = cx_1;
        cl =  (ADDFLAGS16((unsigned short) ecx, si, cx_1)) ? 1 : 0;
        ecx = (ecx >> 16 & 0xffff | (cx_1)) >> 8 & 0xffffff | (cl);
        local1 = (unsigned short) (ecx >> 16 & 0xffff | ((unsigned short) ecx & 0xff)) + ((esi >> 16 & 0xffff | ((unsigned short) esi & 0xff)) + (local4) * 2) * 2;
        local7 = ebx_2;
    }
    return;
}
