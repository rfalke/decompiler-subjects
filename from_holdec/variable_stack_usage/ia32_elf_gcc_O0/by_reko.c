// subject.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080482AC: void _init()
void _init()
{
	if (globals->dw804A004 != 0x00)
		fn08048300();
}

// 080482E0: void __libc_start_main()
void __libc_start_main()
{
}

// 080482F0: void memset()
void memset()
{
}

// 08048300: void fn08048300()
void fn08048300()
{
	word32 esp_3;
	globals->ptr8049FFC();
}

// 08048310: void _start(Register (ptr32 Eq_14) edx, Stack int32 dwArg00)
void _start( * edx, int32 dwArg00)
{
	__align((char *) fp + 0x04);
	__libc_start_main(&globals->t8048560, dwArg00, (char *) fp + 0x04, &globals->t80485A0, &globals->t8048600, edx, fp);
	__hlt();
}

// 08048340: void __x86.get_pc_thunk.bx(Stack word32 dwArg00)
void __x86.get_pc_thunk.bx(word32 dwArg00)
{
}

// 08048350: void deregister_tm_clones()
void deregister_tm_clones()
{
	if (true && 0x00 != 0x00)
	{
		word32 esp_33;
		word32 eax_34;
		byte SCZO_35;
		byte CZ_36;
		byte SZO_37;
		bool C_38;
		bool Z_39;
		word32 ebp_40;
		fn00000000();
	}
}

// 08048380: void register_tm_clones()
void register_tm_clones()
{
	if (0x00 != 0x00 && 0x00 != 0x00)
	{
		word32 esp_41;
		word32 eax_42;
		byte SCZO_43;
		word32 edx_44;
		bool Z_45;
		byte SZO_46;
		bool C_47;
		word32 ebp_48;
		fn00000000();
	}
}

// 080483C0: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if (globals->b804A018 == 0x00)
	{
		deregister_tm_clones();
		globals->b804A018 = 0x01;
	}
}

// 080483E0: void frame_dummy()
void frame_dummy()
{
	if (globals->dw8049F10 != 0x00 && 0x00 != 0x00)
	{
		word32 esp_37;
		word32 eax_38;
		word32 edx_39;
		byte SZO_40;
		bool C_41;
		bool Z_42;
		word32 ebp_43;
		byte SCZO_44;
		fn00000000();
		register_tm_clones();
	}
	else
		register_tm_clones();
}

// 0804840B: void use(Stack (ptr32 word32) dwArg04)
void use(word32 * dwArg04)
{
	globals->dw804A01C += *dwArg04;
}

// 08048422: void fill(Stack (ptr32 word32) dwArg04, Stack ui32 dwArg08)
void fill(word32 * dwArg04, ui32 dwArg08)
{
	memset(dwArg04, 0x78, dwArg08 << 0x02);
}

// 08048442: void with_array(Register ptr32 ebp, Stack ui32 dwArg04)
void with_array(ptr32 ebp, ui32 dwArg04)
{
	union Eq_152 * esp_29 = fp - 0x1C - (uint32) ((uint64) ((uint32) ((dwArg04 << 0x02) + 0x12)) /u 0x10) *s 0x10;
	*(esp_29 - 0x0C) = dwArg04;
	word32 eax_32 = esp_29 + 0x03 >>u 0x02;
	*(esp_29 - (union Eq_152 *) 0x10) = (union Eq_152 *) (eax_32 << 0x02);
	fill(dwArg00, dwArg04);
	*(esp_29 - (union Eq_152 *) 0x10) = (union Eq_152 *) (fp - 0x18);
	use(dwArg00);
	*(esp_29 - (union Eq_152 *) 0x10) = (union Eq_152 *) (eax_32 << 0x02);
	use(dwArg00);
	*(esp_29 - (union Eq_152 *) 0x10) = (union Eq_152 *) (fp - 0x1C);
	use(dwArg00);
}

// 080484DA: void with_alloca(Register ptr32 ebp, Stack ui32 dwArg04)
void with_alloca(ptr32 ebp, ui32 dwArg04)
{
	union Eq_218 * esp_22 = fp - 0x1C - (uint32) ((uint64) ((uint32) ((dwArg04 << 0x02) + 0x1E)) /u 0x10) *s 0x10;
	*(esp_22 - 0x0C) = dwArg04;
	word32 eax_25 = esp_22 + 0x0F >>u 0x04;
	*(esp_22 - (union Eq_218 *) 0x10) = (union Eq_218 *) (eax_25 << 0x04);
	fill(dwArg00, dwArg04);
	*(esp_22 - (union Eq_218 *) 0x10) = (union Eq_218 *) (fp - 0x14);
	use(dwArg00);
	*(esp_22 - (union Eq_218 *) 0x10) = (union Eq_218 *) (eax_25 << 0x04);
	use(dwArg00);
	*(esp_22 - (union Eq_218 *) 0x10) = (union Eq_218 *) (fp - 0x18);
	use(dwArg00);
}

// 08048560: void main(Stack word32 dwArg00, Stack ui32 dwArg04)
void main(word32 dwArg00, ui32 dwArg04)
{
	__align(fp);
	with_alloca(fp - 0x08, dwArg04);
	with_array(fp - 0x08, dwArg04);
}

// 080485A0: void __libc_csu_init(Stack word32 dwArg04, Stack word32 dwArg08, Stack word32 dwArg0C)
void __libc_csu_init(word32 dwArg04, word32 dwArg08, word32 dwArg0C)
{
	_init();
	if (0x0804A0F4 - 0x0804A0F8 >> 0x02 != 0x00)
	{
		do
		{
			word32 esp_65;
			word32 ebp_66;
			word32 edi_67;
			word32 esi_68;
			word32 ebx_69;
			byte SCZO_70;
			word32 eax_71;
			byte SZO_72;
			bool C_73;
			bool Z_74;
			(0x0804A0F8 + 0x0804A000)();
		} while (esi_68 != edi_67 + 0x01);
	}
}

// 08048600: void __libc_csu_fini()
void __libc_csu_fini()
{
}

// 08048604: void _fini()
void _fini()
{
}

