// subject_bss.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

byte g_b411030 = 0x00; // 0000000000411030
word32 g_dw411034 = 0x00; // 0000000000411034
// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400764: void _fini()
void _fini()
{
}

// subject_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw410FE0 = 0x00; // 0000000000410FE0
// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400418: Register word64 _init(Register out ptr64 x30Out)
// Called from:
//      __libc_csu_init
word64 _init(ptr64 & x30Out)
{
	call_weak_fn();
	x30Out = qwArg00;
	return x30;
}

// subject_init_array.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_a410DE0[] = // 0000000000410DE0
	{
	};
// subject_jcr.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word64 g_qw410DF0 = 0x00; // 0000000000410DF0
// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 0000000000400490: void _start(Register (ptr64 Eq_9) x0, Stack Eq_10 qwArg00)
void _start(void (* x0)(), Eq_10 qwArg00)
{
	x0_14 = (uint64) __libc_start_main(g_ptr4004C0, qwArg00, (char *) fp + 8, g_ptr4004C8, g_ptr4004D0, x0, fp);
	abort();
}

int32 (* g_ptr4004C0)(int32 x0, char ** x1, char ** x2) = main; // 00000000004004C0
void (* g_ptr4004C8)() = __libc_csu_init; // 00000000004004C8
void (* g_ptr4004D0)() = __libc_csu_fini; // 00000000004004D0
// 00000000004004D8: void call_weak_fn()
// Called from:
//      _init
void call_weak_fn()
{
	if (g_qw410FE0 == 0x00)
		return;
	__gmon_start__();
}

// 00000000004004F0: void deregister_tm_clones()
// Called from:
//      __do_global_dtors_aux
void deregister_tm_clones()
{
	if (0x00411037 - 0x00411030 <= 0x0E)
		return;
	<anonymous> * x1_12 = g_ptr400528;
	if (x1_12 == null)
		return;
	x1_12();
}

<anonymous> * g_ptr400528 = null; // 0000000000400528
// 0000000000400530: void register_tm_clones()
// Called from:
//      frame_dummy
void register_tm_clones()
{
	int64 x1_7 = 0x00411030 - 0x00411030;
	if ((x1_7 >> 3) + ((x1_7 >> 3) >>u 63) >> 1 == 0x00)
		return;
	<anonymous> * x2_12 = g_ptr400568;
	if (x2_12 == null)
		return;
	word64 x2_16;
	x2_12();
}

<anonymous> * g_ptr400568 = null; // 0000000000400568
// 0000000000400570: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if ((word32) g_b411030 == 0x00)
	{
		deregister_tm_clones();
		g_b411030 = 0x01;
	}
}

// 00000000004005A0: void frame_dummy(Register word64 x29, Register word64 x30)
void frame_dummy(word64 x29, word64 x30)
{
	if (g_qw410DF0 != 0x00)
	{
		<anonymous> * x1_7 = g_ptr4005D8;
		if (x1_7 != null)
		{
			x1_7();
			register_tm_clones();
			return;
		}
	}
	register_tm_clones();
}

<anonymous> * g_ptr4005D8 = null; // 00000000004005D8
// 00000000004005E0: void use(Register (ptr64 word32) x0)
void use(word32 * x0)
{
	g_dw411034 += *x0;
}

// 00000000004005F8: void fill(Register (ptr64 void) x0, Register (ptr64 void) x1)
void fill(void * x0, void * x1)
{
	word32 x1_32_32_7 = SLICE(x1, word32, 32);
	memset(x0, SEQ(x1_32_32_7, 0x78), __sbfiz(x1, 2));
}

// 0000000000400604: void with_array(Sequence word64 x8_32_32_w0, Register word32 x0_32_32, Register word32 x1_32_32, Register Eq_128 w19)
// Called from:
//      main
void with_array(word64 x8_32_32_w0, word32 x0_32_32, word32 x1_32_32, Eq_128 w19)
{
	word32 w0 = (word32) x8_32_32_w0;
	memset(fp + -24, SEQ(x1_32_32, 0x78), __sbfiz(SEQ(x0_32_32, w0), 2));
	g_dw411034 = g_dw411034 + (word32) x19 + 0x0F;
}

// 0000000000400664: Register (ptr64 void) with_alloca(Register (ptr64 void) x0, Register word32 x1_32_32, Register Eq_153 w19, Register out Eq_114 x1Out, Register out ptr64 x8Out, Register out ptr64 x19Out)
// Called from:
//      main
void * with_alloca(void * x0, word32 x1_32_32, Eq_153 w19, union Eq_114 & x1Out, ptr64 & x8Out, ptr64 & x19Out)
{
	Eq_114 x1_26 = SEQ(x1_32_32, 0x78);
	void * x0_27 = memset(fp + -24, x1_26, __sbfiz(x0, 2));
	g_dw411034 = g_dw411034 + (word32) x19 + 0x0F;
	x1Out = x1_26;
	x8Out = 0x00411000;
	x19Out = qwLoc10;
	return x0_27;
}

// 00000000004006BC: void main(Register (ptr64 void) x0, Register word32 x1_32_32, Register word64 x19)
void main(void * x0, word32 x1_32_32, word64 x19)
{
	word32 x19_32_32_18 = SLICE(x19, word32, 32);
	word32 w0 = (word32) x0;
	word64 x1_26;
	Eq_128 x19_27;
	word64 x8_30;
	with_array(SEQ(SLICE(x8_30, word32, 32), (word32) x19_27), SLICE(with_alloca(x0, x1_32_32, SEQ(x19_32_32_18, w0), out x1_26, out x8_30, out x19_27), word32, 32), SLICE(x1_26, word32, 32), x19_27);
}

// 00000000004006E8: void __libc_csu_init(Register word32 w0, Register word64 x1, Register word64 x2, Register word64 x24)
void __libc_csu_init(word32 w0, word64 x1, word64 x2, word64 x24)
{
	word32 x24_32_32_100 = SLICE(x24, word32, 32);
	int64 x20_26 = 4263400 - 0x00410DE0;
	<anonymous> * x21_24[] = g_a410DE0;
	word64 x22_34 = x2;
	word64 x23_38 = x1;
	word64 x30_42;
	word64 x29_41 = _init(out x30_42);
	int64 x20_43 = x20_26 >> 3;
	if (x20_26 >> 3 != 0x00)
	{
		int64 x19_44 = 0x00;
		word64 x24_145 = SEQ(x24_32_32_100, w0);
		do
		{
			word64 x3_74;
			x21_24[x19_44]();
			int64 x31_77 = x20_43 - x19_44;
		} while (x31_77 != 0x00);
	}
}

// 0000000000400760: void __libc_csu_fini()
void __libc_csu_fini()
{
}

