// subject_bss.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

byte g_b804A063 = 0x00; // 0804A063
// subject_data.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

Eq_159 g_t804A030 = // 0804A030
	{
		0.0F
	};
real64 g_r804A040 = 11.0; // 0804A040
Eq_159 global_float = // 0804A048
	{
		10.0F
	};
int64 g_qw804A050 = 5; // 0804A050
int32 global_long = 4; // 0804A058
int32 global_int = 3; // 0804A05C
int16 g_w804A060 = 2; // 0804A060
byte g_b804A062 = 0x01; // 0804A062
// subject_fini.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080488F4: void _fini()
void _fini()
{
}

// subject_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_ptr8049FFC = null; // 08049FFC
// subject_init.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080482AC: void _init()
// Called from:
//      __libc_csu_init
void _init()
{
	if (g_ptr8049FFC != null)
		fn08048300();
}

// subject_init_array.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

<anonymous> * g_a8049F08[] = // 08049F08
	{
	};
// subject_jcr.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word32 g_dw8049F10 = 0x00; // 08049F10
// subject_plt.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 080482E0: void printf()
void printf()
{
}

// 080482F0: void __libc_start_main()
void __libc_start_main()
{
}

// subject_plt_got.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 08048300: void fn08048300()
// Called from:
//      _init
void fn08048300()
{
	g_ptr8049FFC();
}

// subject_rodata.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

word32 _fp_hw = 0x03; // 08048908
word32 _IO_stdin_used = 0x00020001; // 0804890C
char g_str8048914[] = "%f"; // 08048914
char g_str8048917[] = "%d"; // 08048917
char g_str804891A[] = "%zu %zu %zu %zu %zu\n"; // 0804891A
char g_str8048922[] = "%zu %zu %zu\n"; // 08048922
real32 g_r8048930 = 0.0F; // 08048930
real32 g_r8048934 = 3.0F; // 08048934
real32 g_r8048938 = 10.0F; // 08048938
real64 g_r8048940 = 3.141592653589793; // 08048940
real64 g_r8048948 = 12.345; // 08048948
// subject_text.c
// Generated by decompiling subject.exe
// using Reko decompiler version VERSION

#include "subject.h"

// 08048310: void main()
void main()
{
	char ** fp;
	__align_stack<word32>(fp);
	printf("%zu %zu %zu %zu %zu\n", 0x01, 0x02, 0x04, 0x04, 0x08);
	printf("%zu %zu %zu\n", 0x04, 0x08, 0x0C);
}

// 08048355: void _start(Register (ptr32 Eq_40) edx, Stack int32 dwArg00, Stack (ptr32 char) ptrArg04)
void _start(void (* edx)(), int32 dwArg00, char * ptrArg04)
{
	void * fp;
	__align_stack<word32>(&ptrArg04);
	__libc_start_main(&g_t8048310, dwArg00, &ptrArg04, &g_t8048890, &g_t80488F0, edx, fp);
	__halt();
}

// 08048380: void __x86.get_pc_thunk.bx()
void __x86.get_pc_thunk.bx()
{
}

// 08048390: void deregister_tm_clones()
// Called from:
//      __do_global_dtors_aux
void deregister_tm_clones()
{
}

// 080483C0: void register_tm_clones()
// Called from:
//      frame_dummy
void register_tm_clones()
{
}

// 08048400: void __do_global_dtors_aux()
void __do_global_dtors_aux()
{
	if (g_b804A063 == 0x00)
	{
		deregister_tm_clones();
		g_b804A063 = 0x01;
	}
}

// 08048420: void frame_dummy()
void frame_dummy()
{
	register_tm_clones();
}

// 08048450: void use(Stack real64 qwArg04)
void use(real64 qwArg04)
{
	printf("%f", qwArg04);
}

// 08048470: void use_int(Stack int32 dwArg04)
void use_int(int32 dwArg04)
{
	printf("%d", dwArg04);
}

// 08048490: void read_ints()
void read_ints()
{
	printf("%f", (real64) (int16) g_b804A062 + (real64) 0.0F + (real64) g_w804A060 + (real64) global_int + (real64) global_long + (real64) g_qw804A050);
}

// 080484E0: void write_ints(Stack real64 rArg04)
void write_ints(real64 rArg04)
{
	Eq_122 wLoc06_28 = __fstcw();
	Eq_122 ax_11 = SEQ(0x0C, (byte) wLoc06_28);
	__fldcw(ax_11);
	__fldcw(wLoc06_28);
	__fldcw(ax_11);
	__fldcw(wLoc06_28);
	g_b804A062 = (byte) rArg04;
	g_w804A060 = (word16) rArg04;
	__fldcw(ax_11);
	g_qw804A050 = (int64) rArg04;
	__fldcw(wLoc06_28);
	int32 dwLoc10_33 = (int32) rArg04;
	global_int = dwLoc10_33;
	global_long = dwLoc10_33;
}

// 08048550: void read_floats()
void read_floats()
{
	printf("%f", (real64) global_float.u0 + 0.0 + g_r804A040 + (real64) g_t804A030.u1);
}

// 08048590: void write_floats(Stack real64 rArg04)
void write_floats(real64 rArg04)
{
	global_float.u0 = (real32) rArg04;
	g_r804A040 = rArg04;
	g_t804A030.u1 = (real80) rArg04;
}

// 080485B0: void converting_between_floats_f1()
void converting_between_floats_f1()
{
	global_float.u0 = (real32) g_r804A040;
}

// 080485D0: void converting_between_floats_f2()
void converting_between_floats_f2()
{
	global_float.u0 = (real32) g_t804A030.u1;
}

// 080485F0: void converting_between_floats_d1()
void converting_between_floats_d1()
{
	g_r804A040 = (real64) global_float.u0;
}

// 08048600: void converting_between_floats_d2()
void converting_between_floats_d2()
{
	g_r804A040 = (real64) g_t804A030.u1;
}

// 08048620: void converting_between_floats_l1()
void converting_between_floats_l1()
{
	g_t804A030.u1 = (real80) global_float.u0;
}

// 08048630: void converting_between_floats_l2()
void converting_between_floats_l2()
{
	g_t804A030.u1 = (real80) g_r804A040;
}

// 08048640: void basic_operations(Stack real64 rArg04, Stack real64 rArg0C)
void basic_operations(real64 rArg04, real64 rArg0C)
{
	printf("%f", rArg04 + rArg0C);
	printf("%f", rArg04 - rArg0C);
	printf("%f", rArg0C - rArg04);
	printf("%f", rArg04 * rArg0C);
	printf("%f", rArg04 / rArg0C);
	printf("%f", rArg0C / rArg04);
	printf("%f", -rArg04);
}

// 08048700: void compare_floats(Stack Eq_238 rArg04, Stack Eq_238 rArg0C)
void compare_floats(Eq_238 rArg04, Eq_238 rArg0C)
{
	Eq_240 eax_20 = (uint32) (int8) !PARITY_EVEN(rArg0C - rArg04);
	if (rArg0C != rArg04)
		eax_20.u0 = 0x00;
	printf("%d", eax_20);
	Eq_255 eax_53 = (uint32) (int8) PARITY_EVEN(rArg0C - rArg04);
	if (rArg0C != rArg04)
		eax_53.u0 = 0x01;
	printf("%d", eax_53);
	printf("%d", (uint32) (int8) (rArg04 > rArg0C));
	printf("%d", (uint32) (int8) (rArg04 >= rArg0C));
	printf("%d", (word32) (rArg04 > rArg0C));
	printf("%d", (uint32) (int8) (rArg0C >= rArg04));
}

// 080487F0: void constants(Stack real64 rArg04)
void constants(real64 rArg04)
{
	printf("%f", rArg04 + rArg04);
	printf("%f", rArg04 * (real64) 3.0F);
	printf("%f", rArg04 * 3.141592653589793);
	printf("%f", rArg04 * (real64) 10.0F);
	printf("%f", rArg04 * 12.345);
}

// 08048890: void __libc_csu_init(Stack word32 dwArg04, Stack word32 dwArg08, Stack word32 dwArg0C)
void __libc_csu_init(word32 dwArg04, word32 dwArg08, word32 dwArg0C)
{
	_init();
	int32 esi_29 = 0x08049F0C - g_a8049F08;
	if (esi_29 >> 0x02 != 0x00)
	{
		int32 edi_35 = 0x00;
		do
		{
			(*((char *) g_a8049F08 + edi_35 * 0x04))();
			++edi_35;
		} while (esi_29 >> 0x02 != edi_35);
	}
}

// 080488F0: void __libc_csu_fini()
void __libc_csu_fini()
{
}

