...

{
  ...
  while (p != iVar3) {
    p = p + -1;
    putchar((uint)*(byte *)(long)p);
  }
  pvVar4 = (void *)(long)iVar3;
  free(pvVar4);
  return (int)pvVar4;
}
...

{
  ...
  if ((argc == 2) || (argc == 3)) {
    ...
    base = 10;
    if ((2 < argc) &&
       ((base = atoi((char *)(ulong)*(uint *)(long)(argv + 8)), base < 2 || (0x24 < base)))) {
      fwrite("Invalid base\n",1,0xd,stderr);
      return 1;
    }
...

{
  ...
  UNRECOVERED_JUMPTABLE = (code *)UndefinedInstructionException(0,0x100dcc);
